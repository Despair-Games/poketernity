...
describe("Abilities - Imposter", () => {
  ...
  it("should persist transformed attributes across reloads", async () => {
    ...
    expect(game.scene.phaseManager.getCurrentPhase()?.is("CommandPhase")).toBeTruthy();
    ...
    const player = game.field.getPlayerPokemon();
    const moveset = player.getMoveset();
    const speciesForm = player.getSpeciesForm();
    ...
    expect(moveset[0]?.moveId).toBe(MoveId.HIDDEN_POWER);
  });
});
