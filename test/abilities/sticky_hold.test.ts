...
describe("Abilities - Sticky Hold", () => {
  ...
  it.each(
    [MoveId.THIEF, MoveId.PLUCK, MoveId.INCINERATE, MoveId.KNOCK_OFF].map((move) => ({ move, name: MoveId[move] })),
  )("should prevent the user from losing a held item when hit by the move $name", async ({ move }) => {
