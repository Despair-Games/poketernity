...
describe("Abilities - Flame Body/Poison Point/Static", () => {
  ...
  it.each([
    ...
    expect(attacker?.getStatusEffect()).toBe(status);
  });
  ...
  it.each([
    ...
    const attacker = game.scene.getEnemyPokemon();
    expect(attacker?.getStatusEffect()).toBe(StatusEffect.NONE);
  });
  ...
  it("Static can paralyze a Ground-type Pokemon", async () => {
    ...
    expect(attacker?.getStatusEffect()).toBe(StatusEffect.PARALYSIS);
  });
});
