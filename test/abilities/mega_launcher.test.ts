...
describe("Abilities - Mega Launcher", () => {
  ...
  // Note: All affected moves have been verified to have the PULSE_MOVE flag by all_moves
  it("should boost the damage of a pulse move by a factor of 1.5", async () => {
    game.override.moveset(Moves.DARK_PULSE);
    await game.classicMode.startBattle([Species.FEEBAS]);
    const playerPokemon = game.scene.getPlayerPokemon()!;
    const pulseMove = allMoves[Moves.DARK_PULSE];
    vi.spyOn(pulseMove, "calculateBattlePower");
    ...
    expect(pulseMove.calculateBattlePower).toHaveLastReturnedWith(pulseMove.power * 1.5);
  });
