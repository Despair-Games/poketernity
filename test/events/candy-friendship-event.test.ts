...
describe("Candy Friendship Modifier Event", () => {
  ...
  it("should apply the active event's multiplier in classic mode", async () => {
    ...
    expect(starterData.metaProgressionFriendship).toBe(0); // candy friendship
    ...
    expect(starterData.metaProgressionFriendship).toBe(
      FRIENDSHIP_GAIN_FROM_BATTLE * CLASSIC_CANDY_FRIENDSHIP_MULTIPLIER * 3,
    );
  });
  ...
  it("should not apply the event's multiplier when it is not active", async () => {
    ...
    expect(playerPokemon.friendship).toBe(baseFriendship + FRIENDSHIP_GAIN_FROM_BATTLE);
    expect(starterData.metaProgressionFriendship).toBe(
      FRIENDSHIP_GAIN_FROM_BATTLE * CLASSIC_CANDY_FRIENDSHIP_MULTIPLIER,
    );
  });
  ...
  it("should not apply the event's multiplier in daily mode", async () => {
    ...
    expect(playerPokemon.friendship).toBe(baseFriendship + FRIENDSHIP_GAIN_FROM_BATTLE);
    expect(starterData.metaProgressionFriendship).toBe(FRIENDSHIP_GAIN_FROM_BATTLE);
  });
});
