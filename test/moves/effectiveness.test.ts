...
describe("Moves - Type Effectiveness", () => {
  ...
  it("Electric-type attacks are super-effective against Tera-Water Pokemon", () =>
    testMoveEffectiveness(game, MoveId.THUNDERBOLT, Species.EXCADRILL, 2, Abilities.BALL_FETCH, Type.WATER));
  ...
  it("Powder moves have no effect on Tera-Grass Pokemon", () =>
    testMoveEffectiveness(game, MoveId.SLEEP_POWDER, Species.SNORLAX, 0, Abilities.BALL_FETCH, Type.GRASS));
  ...
  it("Prankster-boosted status moves have no effect on Tera-Dark Pokemon", () => {
    game.override.ability(Abilities.PRANKSTER);
    testMoveEffectiveness(game, MoveId.BABY_DOLL_EYES, Species.SNORLAX, 0, Abilities.BALL_FETCH, Type.DARK);
  });
});
