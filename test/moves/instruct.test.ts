...
describe("Moves - Instruct", () => {
  ...
  function instructSuccess(pokemon: Pokemon, move: MoveId): void {
  ...
  it("should respect enemy's status condition", async () => {
    ...
    expect(moveHistory[0].move).toBe(MoveId.SONIC_BOOM);
    expect(moveHistory[1].move).toBe(MoveId.NONE);
    expect(moveHistory[2].move).toBe(MoveId.SONIC_BOOM);
  });
  ...
  it("should not repeat enemy's move through protect", async () => {
    ...
    expect(enemyPokemon.getLastXMoves(-1)[0].move).toBe(MoveId.PROTECT);
  ...
  it("should not repeat enemy's charging move", async () => {
    ...
    enemyPokemon.battleSummonData.moveHistory = [
      { move: MoveId.SONIC_BOOM, targets: [BattlerIndex.PLAYER], result: MoveResult.SUCCESS, virtual: false },
    ];
