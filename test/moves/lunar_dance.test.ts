...
describe("Moves - Lunar Dance", () => {
  ...
  it("should full restore HP, PP and status of switched in pokemon, then fail second use because no remaining backup pokemon in party", async () => {
    ...
    expect(bulbasaur.getStatusEffect()).toBe(StatusEffect.BURN);
    ...
    expect(bulbasaur.getStatusEffect()).toBe(StatusEffect.NONE);
    ...
    expect(rattata.getStatusEffect()).toBe(StatusEffect.BURN);
