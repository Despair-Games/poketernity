...
describe("Moves - Toxic", () => {
  ...
  beforeEach(() => {
    ...
    game.override.battleType("single").moveset(MoveId.TOXIC).enemySpecies(Species.MAGIKARP).enemyMoveset(MoveId.SPLASH);
  });
  ...
  it("should be guaranteed to hit if user is Poison-type", async () => {
    ...
    expect(game.field.getEnemyPokemon().getStatusEffect(true)).toBe(StatusEffect.TOXIC);
  });
