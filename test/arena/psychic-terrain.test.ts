...
describe("Arena - Psychic Terrain", () => {
  ...
  it("blocks increased priority moves", async () => {
    ...
    expect(game.field.getPlayerPokemon()).toHaveUsedMove({ moveId: MoveId.QUICK_ATTACK, result: MoveResult.FAIL });
  ...
  it("blocks single-target status moves that have their priority boosted by Prankster", async () => {
    ...
    expect(game.field.getPlayerPokemon()).toHaveUsedMove({ moveId: MoveId.CHARM, result: MoveResult.FAIL });
  ...
  it("does not block spread-target status moves that have their priority boosted with Prankster", async () => {
    ...
    expect(game.field.getPlayerPokemon()).toHaveUsedMove({ moveId: MoveId.DARK_VOID, result: MoveResult.SUCCESS });
  ...
  it("does not block arena-target status moves that have their priority boosted with Prankster", async () => {
    ...
    expect(game.field.getPlayerPokemon()).toHaveUsedMove({ moveId: MoveId.RAIN_DANCE, result: MoveResult.SUCCESS });
  ...
  it("should not block non-priority moves boosted by Quick Claw", async () => {
    ...
    expect(feebas).toHaveUsedMove({ moveId: MoveId.POUND, result: MoveResult.SUCCESS });
  });
  ...
  it("should block priority moves boosted by Quick Claw", async () => {
    ...
    expect(feebas).toHaveUsedMove({ moveId: MoveId.QUICK_ATTACK, result: MoveResult.FAIL });
  });
});
