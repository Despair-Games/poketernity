...
import { RedirectMoveAbAttr } from "./ab-attrs/redirect-move-ab-attr";
...
export class Move implements Localizable {
  ...
  getEffectScore(user: EnemyPokemon, target: Pokemon, move: Move): number {
    const attrScores = this.attrs
      .map((attr) => attr.getEffectScore(user, target, move))
...
export class AddBattlerTagHeaderAttr extends MoveHeaderAttr {
   ...
   */
  private getShellTrapEffectScore(_user: Pokemon, target: Pokemon, _move: Move): number {
   ...
   */
  private getBeakBlastEffectScore(user: Pokemon, _target: Pokemon, _move: Move): number {
    const affinityBonusChance = 50;
    let affinityBonus = 0;
    user.getOpponents().forEach((opp) => {
      if (
        opp.getEffectiveStat(Stat.ATK) > opp.getEffectiveStat(Stat.SPATK)
    ...
    const statusBonus =
      user.status?.effect && [StatusEffect.SLEEP, StatusEffect.FREEZE].includes(user.status?.effect) ? 1 : 0;
...
export class CounterDamageAttr extends FixedDamageAttr {
   ...
   */
  override getEffectScore(user: EnemyPokemon, _target: Pokemon, _move: Move): number {
...
export class RecoilAttr extends MoveEffectAttr {
  ...
  override getEffectScore(user: EnemyPokemon, _target: Pokemon, _move: Move): number {
    ...
    if (
      (user.hasAbilityWithAttr(BlockRecoilDamageAttr) || user.hasAbilityWithAttr(BlockNonDirectDamageAbAttr))
      && !this.unblockable
    ) {
      return 0;
    } else if (this.useHp) {
...
export class SacrificialAttr extends MoveEffectAttr {
   ...
   */
  override getEffectScore(user: EnemyPokemon, _target: Pokemon, move: Move): number {
    if (user.isBoss()) {
      return -20;
    } else {
      /** The active Pokemon this move can target */
      const activeMoveTargets = getMoveTargets(user, move.id)
        .targets.map((targetIndex) => globalScene.getField()[targetIndex])
      ...
      if (!user.hasTrainer()) {
        ...
        activeMoveTargets
          .filter((pokemon) => pokemon instanceof PlayerPokemon)
          .forEach((opponent) => {
      ...
      activeMoveTargets
        .filter((pokemon) => pokemon === user.getAlly())
        .forEach((ally) => {
...
export class SacrificialAttrOnHit extends MoveEffectAttr {
  ...
  override getEffectScore(user: EnemyPokemon, _target: Pokemon, _move: Move): number {
    if (user.isBoss()) {
      ...
      const hasFavorableMatchup = user
        .getOpponents()
        .some((opp) => opp instanceof PlayerPokemon && user.getMatchupScore(opp) > 3);
...
export class AddSubstituteAttr extends MoveEffectAttr {
   ...
   */
  override getEffectScore(user: EnemyPokemon, _target: Pokemon, _move: Move): number {
    const opposingField = globalScene.getPlayerField();
...
export class PartyStatusCureAttr extends MoveEffectAttr {
   ...
   */
  override getEffectScore(user: EnemyPokemon, _target: Pokemon, _move: Move): number {
    const pokemonToCure = globalScene.getEnemyParty().filter((pokemon) => {
      const statusEffect = pokemon.status?.effect;
      return (
        statusEffect
        && statusEffect !== StatusEffect.FAINT
...
export class SacrificialFullRestoreAttr extends SacrificialAttr {
  ...
  override getEffectScore(user: EnemyPokemon, _target: Pokemon, _move: Move): number {
    if (user.isBoss()) {
      ...
      const allyWithBestMatchup = globalScene
        .getEnemyParty()
...
export class StatusEffectAttr extends MoveEffectAttr {
  ...
  private burnSynergyAbilities = [Abilities.GUTS, Abilities.FLARE_BOOST, Abilities.QUICK_FEET, Abilities.MARVEL_SCALE];
  ...
  protected getBurnEffectScore(user: EnemyPokemon, target: Pokemon, move: Move): number {
    if (target === user.getAlly()) {
      if (move.category === MoveCategory.STATUS) {
        if (
          this.burnSynergyAbilities.some((ab) => target.hasAbility(ab))
  ...
  private userPoisonSynergyAbilities = [Abilities.POISON_PUPPETEER, Abilities.MERCILESS];

  protected getPoisonEffectScore(user: EnemyPokemon, target: Pokemon, move: Move, toxic: boolean) {
    if (target === user.getAlly()) {
      if (move.category === MoveCategory.STATUS) {
        if (
          this.targetPoisonSynergyAbilities.some((ab) => target.hasAbility(ab))
          || target.getMoveset().some((mv) => mv && mv.moveId === Moves.FACADE)
        ) {
...
export class StealHeldItemChanceAttr extends MoveEffectAttr {
  ...
  override getEffectScore(user: EnemyPokemon, target: Pokemon, move: Move): number {
    return this.getTargetHeldItems(target).length > 0 ? this.toChanceBasedScore(user, target, move, 1.5) : 0;
  }
}
...
export class RemoveHeldItemAttr extends MoveEffectAttr {
  ...
  override getEffectScore(user: EnemyPokemon, target: Pokemon, _move: Move): number {
    return this.getTargetHeldItems(target).length ? this.getRandomScore(user, 1, 50) : 0;
  }
}
...
export class EatBerryAttr extends MoveEffectAttr {
  ...
  override getEffectScore(user: EnemyPokemon, _target: Pokemon, move: Move): number {
    ...
    const berrySynergyAbilities = [Abilities.HARVEST, Abilities.CUD_CHEW, Abilities.RIPEN];

    let totalScore = 0;
    targets
      .map((battlerIndex) => globalScene.getField(true).find((p) => p.getBattlerIndex() === battlerIndex))
      .forEach((pokemon) => {
        if (pokemon && this.getTargetHeldBerries(pokemon).length > 0) {
          totalScore +=
            (user.isOpponent(pokemon) ? -1 : 1) * (berrySynergyAbilities.some((ab) => pokemon.hasAbility(ab)) ? 2 : 1);
        }
      });
...
export class BypassSleepAttr extends MoveAttr {
  ...
  override getEffectScore(user: EnemyPokemon, _target: Pokemon, _move: Move): number {
    return user.status && user.status.effect === StatusEffect.SLEEP ? 3 + (user.status.sleepTurnsRemaining ?? 0) : 0;
  }
}
...
export class WeatherChangeAttr extends MoveEffectAttr {
  ...
  override getEffectScore(_user: EnemyPokemon, _target: Pokemon, _move: Move): number {
    ...
    const userWeatherBenefit = globalScene.arena.getPartyBenefitFromWeather(this.weatherType, false);
...
export class ClearWeatherAttr extends MoveEffectAttr {
  ...
  override getEffectScore(user: EnemyPokemon, _target: Pokemon, _move: Move): number {
    ...
    const userWeatherBenefit = globalScene.arena.getPartyBenefitFromWeather(this.weatherType, false);
    const oppWeatherBenefit = globalScene.arena.getPartyBenefitFromWeather(this.weatherType, true);

    return userWeatherBenefit < oppWeatherBenefit ? 1 + this.getRandomScore(user, 1, 40) : 0;
  }
}
...
export class TerrainChangeAttr extends MoveEffectAttr {
  ...
  override getEffectScore(_user: EnemyPokemon, _target: Pokemon, _move: Move): number {
    if (globalScene.arena.terrain?.terrainType === this.terrainType) {
      return -5;
    } else {
      return Math.min(globalScene.arena.getPartyBenefitFromTerrain(this.terrainType, false), 3);
    }
  }
}
...
export class ClearTerrainAttr extends MoveEffectAttr {
  ...
  override getEffectScore(user: EnemyPokemon, _target: Pokemon, _move: Move): number {
    // TODO: Change this so the AI "cheats" less (not revealing player party details)
    const currentTerrain = globalScene.arena.terrain?.terrainType;
    if (isNullOrUndefined(currentTerrain)) {
      return 0;
    }
    const userTerrainBenefit = globalScene.arena.getPartyBenefitFromTerrain(currentTerrain, false);
    const oppTerrainBenefit = globalScene.arena.getPartyBenefitFromTerrain(currentTerrain, true);

    return userTerrainBenefit < oppTerrainBenefit ? 1 + this.getRandomScore(user, 1, 40) : 0;
  }
}
...
export class AwaitCombinedPledgeAttr extends OverrideMoveEffectAttr {
  ...
  override getEffectScore(user: EnemyPokemon, _target: Pokemon, move: Move): number {
    ...
    const allyMoves = user
      .getAlly()
      .getMoveset()
      .filter((mv) => !isNullOrUndefined(mv))
      .map((mv) => mv.getMove());
...
export class StatStageChangeAttr extends MoveEffectAttr {
   ...
   */
  getOffensiveStatStageChangeEffectScore(
    user: EnemyPokemon,
    target: Pokemon,
   ...
   * @returns a base score to be modified by effect chance for a final score.
   */
  getDefensiveStatStageChangeEffectScore(
    user: EnemyPokemon,
    target: Pokemon,
    statStageChange: number,
    isPhysical: boolean,
    ...
    const numAffectedOpponents = (
      affectedPokemon.isPlayer() ? globalScene.getEnemyField() : globalScene.getPlayerField()
    ).filter((opp) => opp.getEffectiveStat(affectedStat) > opp.getEffectiveStat(unaffectedStat)).length;
  ...
  getSpeedStatStageChangeEffectScore(user: EnemyPokemon, target: Pokemon, statStageChange: number): number {
    ...
    if (!affectedPokemon.isPlayer()) {
      const opponentsToBeOutsped = globalScene.getPlayerField().filter((opp) => {
      ...
      const alliesToOutspeed = globalScene.getEnemyField().filter((ally) => {
  ...
  getAccuracyStatStageChangeEffectScore(user: EnemyPokemon, target: Pokemon, statStageChange: number): number {
    ...
    if (!affectedPokemon.isPlayer()) {
      ...
      const inaccurateMoves = affectedPokemon.getMoveset().filter((mv) => mv && mv.getMove().accuracy <= 85);
  ...
  getEvasivenessStatStageChangeEffectScore(user: EnemyPokemon, target: Pokemon, statStageChange: number): number {
    ...
    if (!affectedPokemon.isPlayer()) {
      ...
      const allyHasLowAccuracy = globalScene
        .getEnemyField()
        .some((ally) => ally.getMoveset().some((mv) => mv && mv.getMove().accuracy <= 85));
...
export class SecretPowerAttr extends MoveEffectAttr {
  ...
  private determineBiomeEffect(biome: Biome): MoveEffectAttr {
    ...
    switch (biome) {
      ...
      case Biome.ABYSS:
      case Biome.SPACE:
        secondaryEffect = new FlinchAttr();
        break;
      case Biome.END:
  ...
  override getEffectScore(user: EnemyPokemon, target: Pokemon, move: Move): number {
    const secondaryEffect =
      this.determineTerrainEffect(globalScene.arena.terrain?.terrainType ?? TerrainType.NONE)
      ?? this.determineBiomeEffect(globalScene.arena.biomeType);
...
export class PostVictoryStatStageChangeAttr extends MoveAttr {
  ...
  constructor(stats: BattleStat[], stages: number, condition?: MoveConditionFunc, showMessage: boolean = true) {
    super(true, { scoresOnKO: true });
    this.stats = stats;
    this.stages = stages;
    this.condition = condition;
...
export class CutHpStatStageBoostAttr extends StatStageChangeAttr {
   ...
   */
  override getEffectScore(user: EnemyPokemon, _target: Pokemon, _move: Move): number {
    const oppThreatLevels = globalScene.getPlayerField().map((opp) => {
    ...
    const totalStatStagesChanged = this.stats
      .map((stat) => {
  ...
  override getEffectScore(_user: EnemyPokemon, target: Pokemon, _move: Move): number {
    if (this.targetAllPokemon) {
      ...
      activePokemon.forEach(
        (pokemon) => (totalStatStageChange += (pokemon.isPlayer() ? -1 : 1) * this.getStatStageChange(pokemon)),
      );
...
export class SwapStatStagesAttr extends MoveEffectAttr {
   ...
   */
  override getEffectScore(user: EnemyPokemon, target: Pokemon, _move: Move): number {
    const targetStatStages = this.stats.map((stat) => target.getStatStage(stat)).reduce((total, ssc) => total + ssc);

    const userStatStages = this.stats.map((stat) => user.getStatStage(stat)).reduce((total, ssc) => total + ssc);
...
const doublePowerChanceMessageFunc = (user: Pokemon, _target: Pokemon, move: Move) => {
  let message: string = "";
...
export class LastMoveDoublePowerAttr extends VariablePowerAttr {
   ...
   */
  override getEffectScore(user: EnemyPokemon, _target: Pokemon, _move: Move): number {
    if (!globalScene.currentBattle.double || !user.getAlly()?.isActive(true)) {
      return 0;
    }

    const allyHasFusionMove = user
      .getAlly()
      .getMoveset()
...
export class CrashDamageScoreAttr extends MoveAttr {
  ...
  override getEffectScore(user: EnemyPokemon, target: Pokemon, _move: Move): number {
    const targetKnowsProtectMove = target.getMoveset().some((mv) => mv && mv.getMove().hasAttr(ProtectAttr));
...
export class BypassRedirectAttr extends MoveAttr {
   ...
   */
  override getEffectScore(user: EnemyPokemon, target: Pokemon, _move: Move): number {
    const possibleRedirects = globalScene.getField(true).filter((pokemon) => pokemon !== user && pokemon !== target);

    const canRedirect = possibleRedirects.some(
      (pokemon) =>
...
export class AddBattlerTagAttr extends MoveEffectAttr {
  ...
  protected get turnCountMin() {
    return this.options?.turnCountMin ?? 0;
  }

  protected get turnCountMax() {
    return this.options?.turnCountMax ?? this.turnCountMin;
  }

  protected get cancelOnFail() {
    return this.options?.cancelOnFail ?? false;
  }

  protected get failOnOverlap() {
  ...
  protected getTagScore(_user: EnemyPokemon, _target: Pokemon, move: Move): number {
...
/**
 * Attribute for the move {@link https://bulbapedia.bulbagarden.net/wiki/Disable_(move) | Disable}.
 * Disables the last move used by the target for 4 turns.
 */
export class DisableAttr extends AddBattlerTagAttr {
  ...
  protected override getTagScore(user: EnemyPokemon, target: Pokemon, _move: Move): number {
    const targetBestAttack = target
      .getAttackMoves(true, true)
      .sort(
        (aMove, bMove) =>
          target.getExpectedAttackScore(user, bMove.getMove()) - target.getExpectedAttackScore(user, aMove.getMove()),
      )[0];

    const affectedMove = target
      .getLastXMoves(-1)
      .find((m) => m.move !== Moves.NONE && m.move !== Moves.STRUGGLE && !m.virtual)?.move;

    if (targetBestAttack.moveId === affectedMove) {
      const outspeed = user.getEffectiveStat(Stat.SPD) > target.getEffectiveStat(Stat.SPD);
      return 1 + (outspeed ? 1 : 0);
    }
    return 0;
  }
...
/**
 * Attribute for the move {@link https://bulbapedia.bulbagarden.net/wiki/Torment_(move) | Torment}.
 * Prevents the target from selecting the same move twice in a row.
 */
...
/**
 * Attribute for the move {@link https://bulbapedia.bulbagarden.net/wiki/Taunt_(move) | Taunt}.
 * Prevents the target from selecting and using Status moves.
 */
export class TauntAttr extends AddBattlerTagAttr {
  ...
  protected override getTagScore(user: EnemyPokemon, target: Pokemon, _move: Move): number {
    ...
    return targetMaxEAS <= 1 ? 1 + this.getRandomScore(user, 1, 60) : 0;
  }
}

/**
 * Attribute for the moves {@link https://bulbapedia.bulbagarden.net/wiki/Heal_Block_(move) | Heal Block}
 * and {@link https://bulbapedia.bulbagarden.net/wiki/Psychic_Noise_(move) | Psychic Noise}.
 * Prevents the target from restoring HP and selecting healing moves.
 */
export class HealBlockAttr extends AddBattlerTagAttr {
  constructor(failOnOverlap: boolean, turnCount: number) {
    super(BattlerTagType.HEAL_BLOCK, false, {
      failOnOverlap: failOnOverlap,
...
export class ThroatChopAttr extends AddBattlerTagAttr {
  ...
  protected override getTagScore(user: EnemyPokemon, target: Pokemon, _move: Move): number {
    const targetAttacksByEAS = target
      .getEstimatedAttackMoves()
      .sort((aMove, bMove) => target.getExpectedAttackScore(user, bMove) - target.getExpectedAttackScore(user, aMove));

    if (targetAttacksByEAS[0].hasFlag(MoveFlags.SOUND_BASED)) {
      return 1;
    } else {
      return 0;
    }
...
/**
 * Attribute for the move {@link https://bulbapedia.bulbagarden.net/wiki/Minimize_(move) | Minimize}.
 * Adds a "minimized" tag to the user that causes the user to receive double damage
 * from certain moves.
 */
export class MinimizeAttr extends AddBattlerTagAttr {
  constructor() {
    super(BattlerTagType.MINIMIZED, true, { failOnOverlap: true });
  }

  protected override getTagScore(_user: EnemyPokemon, _target: Pokemon, _move: Move): number {
    return 0;
  }
}
...
export class LeechSeedAttr extends AddBattlerTagAttr {
  ...
  protected override getTagScore(user: EnemyPokemon, target: Pokemon, _move: Move): number {
    ...
    return (targetMaxEAS <= 1 ? 1 : 0) + this.getRandomScore(user, 1, 70);
  }
}

/**
 * Attribute for the move {@link https://bulbapedia.bulbagarden.net/wiki/Focus_Energy_(move) | Focus Energy}.
...
/**
 * Attribute for moves that trap the target (e.g. Mean Look and Spider Web).
 * @see {@linkcode BindAttr} for damaging trap effects.
 */
export class TrapAttr extends AddBattlerTagAttr {
  constructor(options?: AddBattlerTagAttrOptions) {
...
/**
 * Attribute for the move {@link https://bulbapedia.bulbagarden.net/wiki/Nightmare_(move) | Nightmare}.
 * Deals damage to the target at the end of each turn while the target is asleep.
 */
...
/**
 * Attribute for the move {@link https://bulbapedia.bulbagarden.net/wiki/Attract_(move) | Attract}.
 * Causes the target to become infatuated with the user.
 */
export class AttractAttr extends AddBattlerTagAttr {
  constructor() {
  ...
  override getCondition(): MoveConditionFunc {
    return (user, target, _move) => user.isOppositeGender(target);
  }
}
...
export class EncoreAttr extends AddBattlerTagAttr {
  ...
  protected override getTagScore(user: EnemyPokemon, target: Pokemon, _move: Move): number {
    ...
    const targetLastMove = target.getLastXMoves(1)[0].move;

    if (outspeed && allMoves[targetLastMove].category === MoveCategory.STATUS) {
      return 2;
    } else {
      return 0;
    }
  ...
  override getCondition(): MoveConditionFunc {
    return (user, target, _move) => new EncoreTag(user.id).canAdd(target);
  }
}
...
export class StockpileAttr extends AddBattlerTagAttr {
  ...
  override getCondition(): MoveConditionFunc {
    return (user, _target, _move) => (user.getTag(StockpilingTag)?.stockpiledCount ?? 0) < 3;
  }
}
...
export class CenterOfAttentionAttr extends AddBattlerTagAttr {
  ...
  protected override getTagScore(user: EnemyPokemon, _target: Pokemon, _move: Move): number {
    if (this.selfTarget) {
      ...
      const ally = user.getAlly() as EnemyPokemon;
      const activeOpponents = globalScene.getPlayerField().filter((opp) => opp.isActive(true));
      const allyEAS = activeOpponents
        .map((opp) => ally.getAttackMoves(true).map((mv) => ally.getExpectedAttackScore(opp, mv.getMove())))
        .flat();

      if (ally.getAverageMatchupScore() <= 3 && allyEAS.some((eas) => eas >= 4)) {
        return 2;
      } else {
        return this.getRandomScore(user, 1, 20);
      }
    } else {
      return 0; // AI isn't smart enough to use Spotlight effectively...
    }
  }
}

export class ChargeTagAttr extends AddBattlerTagAttr {
  constructor() {
...
export class IngrainAttr extends AddBattlerTagAttr {
  ...
  protected override getTagScore(user: EnemyPokemon, _target: Pokemon, _move: Move): number {
    const activeOpponents = globalScene.getPlayerField().filter((opp) => opp.isActive(true));
    const oppEAS = activeOpponents
      .map((opp) => opp.getEstimatedAttackMoves().map((mv) => opp.getExpectedAttackScore(user, mv)))
      .flat();

    if (Math.max(...oppEAS) <= 1) {
      return this.getRandomScore(user, 1, user.isOfType(Type.GHOST) ? 75 : 55);
    } else {
      return 0;
    }
  }
...
export class YawnAttr extends AddBattlerTagAttr {
  ...
  override getCondition(): MoveConditionFunc {
    return (user, target, _move) => !target.status && !target.isSafeguarded(user);
  }
}
...
export class RoostAttr extends AddBattlerTagAttr {
  ...
  protected override getTagScore(user: EnemyPokemon, _target: Pokemon, _move: Move): number {
    ...
    const oppHasSuperEffectiveAttack = globalScene
      .getPlayerField()
      .filter((opp) => opp.isActive(true))
      .filter((opp) => user.getEffectiveStat(Stat.SPD) > opp.getEffectiveStat(Stat.SPD))
      .some((opp) =>
        opp.getEstimatedAttackMoves().some((mv) => getTypeDamageMultiplier(opp.getMoveType(mv), Type.FLYING) >= 2),
      );

    if (isFlying && oppHasSuperEffectiveAttack) {
...
export class AquaRingAttr extends AddBattlerTagAttr {
  ...
  protected override getTagScore(user: EnemyPokemon, _target: Pokemon, _move: Move): number {
    ...
    if (Math.max(...oppEAS) <= 1) {
      return this.getRandomScore(user, 1, 75);
    } else {
      return 0;
    }
  }
...
export class FloatingAttr extends AddBattlerTagAttr {
  constructor(selfTarget: boolean, turnCount: number) {
  ...
  protected override getTagScore(user: EnemyPokemon, target: Pokemon, _move: Move): number {
    ...
    const opposingField = (allyTarget ? globalScene.getPlayerField() : globalScene.getEnemyField()).filter((pokemon) =>
      pokemon.isActive(true),
    );

    let totalScore = 0;
    opposingField.forEach((opp) => {
      const oppAttacks = allyTarget
        ? opp.getEstimatedAttackMoves()
        : opp.getAttackMoves(true).map((mv) => mv.getMove());
      if (oppAttacks.some((mv) => opp.getMoveType(mv) === Type.GROUND)) {
        totalScore += this.getRandomScore(user, 1, 40);
      }
    });
...
export class GroundingAttr extends AddBattlerTagAttr {
  ...
  protected override getTagScore(user: EnemyPokemon, target: Pokemon, move: Move): number {
    ...
    if (targetNonFlyingTypes.length > 0) {
      ...
      const postEffectAttackEffectiveness = targetNonFlyingTypes
        .map((type) => getTypeDamageMultiplier(moveType, type) as number)
        .reduce((a, b) => a * b);

      if (postEffectAttackEffectiveness >= 2) {
        return 1;
      } else if (postEffectAttackEffectiveness < 1) {
        return -1;
      }
    }
    return 0;
...
export class InterruptingAttr extends AddBattlerTagAttr {
  constructor() {
...
export class TarShotAttr extends AddBattlerTagAttr {
  ...
  protected override getTagScore(user: EnemyPokemon, _target: Pokemon, _move: Move): number {
    const userField = globalScene.getEnemyField().filter((p) => p.isActive(true));
    const hasFireMove = userField.some((p) =>
      p.getAttackMoves(true).some((mv) => p.getMoveType(mv.getMove()) === Type.FIRE),
    );

    return hasFireMove ? this.getRandomScore(user, 1, 40) : 0;
  }
}
...
export class OctolockAttr extends AddBattlerTagAttr {
  ...
  override getTagScore(user: EnemyPokemon, target: Pokemon, _move: Move): number {
    if (target instanceof PlayerPokemon && user.getMatchupScore(target) >= 4) {
...
export class ElectrifyAttr extends AddBattlerTagAttr {
  ...
  protected override getTagScore(user: EnemyPokemon, target: Pokemon, _move: Move): number {
    ...
    if (outspeed) {
      const sideHasLightningRod = globalScene
        .getEnemyField()
        .filter((p) => p.isActive(true))
        .some((p) => p.hasAbility(Abilities.LIGHTNING_ROD));

      const userHasImmuneAb = [Abilities.VOLT_ABSORB, Abilities.MOTOR_DRIVE].some((ab) => user.hasAbility(ab));
...
export class TelekinesisAttr extends AddBattlerTagAttr {
  ...
  protected override getTagScore(user: EnemyPokemon, _target: Pokemon, _move: Move): number {
    const allyField = globalScene.getEnemyField().filter((p) => p.isActive(true));
...
export class DragonCheerAttr extends AddBattlerTagAttr {
  ...
  protected override getTagScore(user: EnemyPokemon, target: Pokemon, _move: Move): number {
    ...
    return this.getRandomScore(user, 1, 55) + (isDragon ? 1 : 0) + (hasHighCritMove || hasHighCritAb ? 1 : 0);
  }
}
...
export class GulpMissileTagAttr extends MoveEffectAttr {
  ...
  override getEffectScore(user: EnemyPokemon, _target: Pokemon, _move: Move): number {
    const isCramorant =
      user.hasAbility(Abilities.GULP_MISSILE) && user.species.speciesId === Species.CRAMORANT && user.formIndex === 0; // base form, no gulp missile stored
...
export class LapseBattlerTagAttr extends MoveEffectAttr {
  ...
  override getEffectScore(user: EnemyPokemon, target: Pokemon, _move: Move): number {
    ...
    const affectedTags = affectedPokemon.summonData?.tags.filter((tag) => this.tagTypes.includes(tag.tagType));
...
export class ConfuseAttr extends AddBattlerTagAttr {
  constructor(selfTarget?: boolean) {
    super(BattlerTagType.CONFUSED, selfTarget, {
      turnCountMin: 2,
      turnCountMax: 5,
    });
  }
...
export class BindAttr extends AddBattlerTagAttr {
  constructor(tagType: BattlerTagType) {
    super(tagType, false, {
      turnCountMin: 4,
      turnCountMax: 5,
...
export class ProtectAttr extends AddBattlerTagAttr {
  ...
  private getContactEffectScore(user: EnemyPokemon): number {
    const numOppsWithPhysAffinity = globalScene
      .getEnemyField()
      .filter((p) => p.isActive(true) && p.getEffectiveStat(Stat.ATK) > p.getEffectiveStat(Stat.SPATK)).length;
...
export function initMoves() {
  allMoves.push(
    ...
    new StatusMove(Moves.DISABLE, Type.NORMAL, 100, 20, -1, 0, 1).attr(DisableAttr).ignoresSubstitute(),
    ...
    new StatusMove(Moves.REFLECT, Type.PSYCHIC, -1, 20, -1, 0, 1)
      ...
      .target(MoveTarget.USER_SIDE),
    new SelfStatusMove(Moves.FOCUS_ENERGY, Type.NORMAL, -1, 30, -1, 0, 1).attr(FocusEnergyAttr),
    ...
    new StatusMove(Moves.SPIDER_WEB, Type.BUG, -1, 10, -1, 0, 2)
      ...
      .attr(TrapAttr, { failOnOverlap: true }),
    ...
    new StatusMove(Moves.MEAN_LOOK, Type.NORMAL, -1, 5, -1, 0, 2)
      .condition(failIfGhostTypeCondition)
      .attr(TrapAttr, { failOnOverlap: true }),
    new StatusMove(Moves.ATTRACT, Type.NORMAL, 100, 15, -1, 0, 2).attr(AttractAttr).ignoresSubstitute(),
    ...
    new AttackMove(Moves.WHIRLPOOL, Type.WATER, MoveCategory.SPECIAL, 35, 85, 15, -1, 0, 2)
      .attr(BindAttr, BattlerTagType.WHIRLPOOL)
      .attr(HitsTagForDoubleDamageAttr, BattlerTagType.UNDERWATER),
    ...
    new SelfStatusMove(Moves.STOCKPILE, Type.NORMAL, -1, 20, -1, 0, 3).attr(StockpileAttr),
    ...
    new StatusMove(Moves.HAIL, Type.ICE, -1, 10, -1, 0, 3)
      ...
      .target(MoveTarget.BOTH_SIDES),
    new StatusMove(Moves.TORMENT, Type.DARK, 100, 15, -1, 0, 3).attr(TormentAttr).ignoresSubstitute().edgeCase(), // Incomplete implementation because of Uproar's partial implementation
    ...
    new SelfStatusMove(Moves.FOLLOW_ME, Type.NORMAL, -1, 20, -1, 2, 3).attr(CenterOfAttentionAttr),
    ...
    new SelfStatusMove(Moves.CHARGE, Type.ELECTRIC, -1, 20, -1, 0, 3)
      ...
      .attr(ChargeTagAttr),
    new StatusMove(Moves.TAUNT, Type.DARK, 100, 20, -1, 0, 3).attr(TauntAttr).ignoresSubstitute(),
    new StatusMove(Moves.HELPING_HAND, Type.NORMAL, -1, 20, -1, 5, 3)
      .attr(HelpingHandAttr)
      .ignoresSubstitute()
    ...
    new StatusMove(Moves.YAWN, Type.NORMAL, -1, 10, -1, 0, 3).attr(YawnAttr),
    ...
    new AttackMove(Moves.SAND_TOMB, Type.GROUND, MoveCategory.PHYSICAL, 35, 85, 15, -1, 0, 3)
      .attr(BindAttr, BattlerTagType.SAND_TOMB)
      .makesContact(false),
    ...
    new SelfStatusMove(Moves.ROOST, Type.FLYING, -1, 5, -1, 0, 4).attr(HealAttr, 0.5).attr(RoostAttr).triageMove(),
    ...
    new StatusMove(Moves.HEART_SWAP, Type.PSYCHIC, -1, 10, -1, 0, 4)
      ...
      .ignoresSubstitute(),
    new SelfStatusMove(Moves.AQUA_RING, Type.WATER, -1, 20, -1, 0, 4).attr(AquaRingAttr),
    new SelfStatusMove(Moves.MAGNET_RISE, Type.ELECTRIC, -1, 10, -1, 0, 4)
      .attr(FloatingAttr, true, 5)
      .condition(
        (user, _target, _move) =>
          !globalScene.arena.getTag(ArenaTagType.GRAVITY)
    ...
    new SelfStatusMove(Moves.RAGE_POWDER, Type.BUG, -1, 20, -1, 2, 5).powderMove().attr(CenterOfAttentionAttr),
    ...
    new StatusMove(Moves.MISTY_TERRAIN, Type.FAIRY, -1, 10, -1, 0, 6)
      ...
      .target(MoveTarget.BOTH_SIDES),
    new StatusMove(Moves.ELECTRIFY, Type.ELECTRIC, -1, 20, -1, 0, 6).attr(ElectrifyAttr),
    ...
    new AttackMove(Moves.INFESTATION, Type.BUG, MoveCategory.SPECIAL, 20, 100, 20, -1, 0, 6)
      .makesContact()
      .attr(BindAttr, BattlerTagType.INFESTATION),
    ...
    new AttackMove(Moves.THOUSAND_WAVES, Type.GROUND, MoveCategory.PHYSICAL, 90, 100, 10, -1, 0, 6)
      .attr(TrapAttr, { lastHitOnly: true })
      .makesContact(false)
      .target(MoveTarget.ALL_NEAR_ENEMIES),
    ...
    new StatusMove(Moves.SPOTLIGHT, Type.NORMAL, -1, 15, -1, 3, 7)
      .attr(CenterOfAttentionAttr, false)
      .condition(failIfSingleBattle),
    new StatusMove(Moves.TOXIC_THREAD, Type.POISON, 100, 20, -1, 0, 7)
      .attr(StatusEffectAttr, StatusEffect.POISON)
      .attr(StatStageChangeAttr, [Stat.SPD], -1),
    new SelfStatusMove(Moves.LASER_FOCUS, Type.NORMAL, -1, 30, -1, 0, 7).attr(LaserFocusAttr),
    ...
    new AttackMove(Moves.THROAT_CHOP, Type.DARK, MoveCategory.PHYSICAL, 80, 100, 15, 100, 0, 7).attr(ThroatChopAttr),
    new AttackMove(Moves.POLLEN_PUFF, Type.BUG, MoveCategory.SPECIAL, 90, 100, 15, -1, 0, 7)
      .attr(StatusCategoryOnAllyAttr)
      .attr(HealOnAllyAttr, 0.5, true, false)
      .ballBombMove(),
    new AttackMove(Moves.ANCHOR_SHOT, Type.STEEL, MoveCategory.PHYSICAL, 80, 100, 20, 100, 0, 7).attr(TrapAttr, {
      lastHitOnly: true,
    }),
    ...
    new AttackMove(Moves.GLAIVE_RUSH, Type.DRAGON, MoveCategory.PHYSICAL, 120, 100, 5, -1, 0, 9).attr(GlaiveRushAttr),
