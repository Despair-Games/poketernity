...
export abstract class MoveRestrictionBattlerTag extends BattlerTag {
  ...
  override lapse(pokemon: Pokemon, lapseType: BattlerTagLapseType): boolean {
    if (lapseType === BattlerTagLapseType.PRE_MOVE) {
      ...
      const phase = phaseManager.getCurrentPhase() as MovePhase;
      const move = phase.move;

      if (this.isMoveRestricted(move.moveId, pokemon)) {
        if (this.interruptedText(pokemon, move.moveId)) {
          queueMessage(this.interruptedText(pokemon, move.moveId));
        }
        phase.cancel();
      }
...
export class BeakBlastChargingTag extends BattlerTag {
  ...
  override onAdd(pokemon: Pokemon): void {
    ...
    queueMessage(i18next.t("moveTriggers:startedHeatingUpBeak", { pokemonName: getPokemonNameWithAffix(pokemon) }));
  }
...
export class ShellTrapTag extends BattlerTag {
  ...
  override onAdd(pokemon: Pokemon): void {
    queueMessage(i18next.t("moveTriggers:setUpShellTrap", { pokemonName: getPokemonNameWithAffix(pokemon) }));
  }
  ...
  override lapse(pokemon: Pokemon, lapseType: BattlerTagLapseType): boolean {
    if (lapseType === BattlerTagLapseType.AFTER_HIT) {
      ...
      if (phaseData?.move.category === MoveCategory.PHYSICAL && pokemon.isOpponent(phaseData.attacker)) {
        const shellTrapPhaseIndex = phaseManager.phaseQueue.findIndex(
        ...
        if (shellTrapPhaseIndex !== -1 && shellTrapPhaseIndex !== firstMovePhaseIndex) {
          const shellTrapMovePhase = phaseManager.phaseQueue.splice(shellTrapPhaseIndex, 1)[0];
          phaseManager.prependToPhase(shellTrapMovePhase, MovePhase);
        }
...
export class TrappedTag extends BattlerTag {
  ...
  override onAdd(pokemon: Pokemon): void {
    ...
    queueMessage(this.getTrapMessage(pokemon));
  }
...
export class FlinchedTag extends BattlerTag {
  ...
  override lapse(pokemon: Pokemon, lapseType: BattlerTagLapseType): boolean {
    if (lapseType === BattlerTagLapseType.PRE_MOVE) {
      (phaseManager.getCurrentPhase() as MovePhase).cancel();
      queueMessage(i18next.t("battlerTags:flinchedLapse", { pokemonNameWithAffix: getPokemonNameWithAffix(pokemon) }));
    }
...
export class ConfusedTag extends BattlerTag {
  ...
  override onAdd(pokemon: Pokemon): void {
    ...
    phaseManager.unshiftPhase(new CommonAnimPhase(pokemon.getBattlerIndex(), undefined, CommonAnim.CONFUSION));
    queueMessage(i18next.t("battlerTags:confusedOnAdd", { pokemonNameWithAffix: getPokemonNameWithAffix(pokemon) }));
  }

  override onRemove(pokemon: Pokemon): void {
    super.onRemove(pokemon);

    queueMessage(i18next.t("battlerTags:confusedOnRemove", { pokemonNameWithAffix: getPokemonNameWithAffix(pokemon) }));
  }
  ...
  override lapse(pokemon: Pokemon, lapseType: BattlerTagLapseType): boolean {
    ...
    if (ret) {
      queueMessage(i18next.t("battlerTags:confusedLapse", { pokemonNameWithAffix: getPokemonNameWithAffix(pokemon) }));
      phaseManager.unshiftPhase(new CommonAnimPhase(pokemon.getBattlerIndex(), undefined, CommonAnim.CONFUSION));
      ...
      if (pokemon.randSeedInt(3) === 0) {
        ...
        queueMessage(i18next.t("battlerTags:confusedLapseHurtItself"));
...
export class InfatuatedTag extends BattlerTag {
  ...
  override lapse(pokemon: Pokemon, lapseType: BattlerTagLapseType): boolean {
    ...
    if (ret) {
      ...
      phaseManager.unshiftPhase(new CommonAnimPhase(pokemon.getBattlerIndex(), undefined, CommonAnim.ATTRACT));
...
export class SeedTag extends BattlerTag {
  ...
  override onAdd(pokemon: Pokemon): void {
    ...
    queueMessage(i18next.t("battlerTags:seededOnAdd", { pokemonNameWithAffix: getPokemonNameWithAffix(pokemon) }));
...
export class PowderTag extends BattlerTag {
  ...
  override onAdd(pokemon: Pokemon): void {
    ...
    queueMessage(i18next.t("battlerTags:powderOnAdd", { pokemonNameWithAffix: getPokemonNameWithAffix(pokemon) }));
  }
  ...
  override lapse(pokemon: Pokemon, lapseType: BattlerTagLapseType): boolean {
    if (lapseType === BattlerTagLapseType.PRE_MOVE) {
      const movePhase = phaseManager.getCurrentPhase();
      if (movePhase instanceof MovePhase) {
        ...
        if (
          ...
          queueMessage(i18next.t("battlerTags:powderLapse", { moveName: move.name }));
        }
      }
      return true;
    } else {
...
export class NightmareTag extends BattlerTag {
  ...
  override onAdd(pokemon: Pokemon): void {
    ...
    queueMessage(i18next.t("battlerTags:nightmareOnAdd", { pokemonNameWithAffix: getPokemonNameWithAffix(pokemon) }));
  }
  ...
  override lapse(pokemon: Pokemon, lapseType: BattlerTagLapseType): boolean {
    ...
    if (ret) {
      queueMessage(i18next.t("battlerTags:nightmareLapse", { pokemonNameWithAffix: getPokemonNameWithAffix(pokemon) }));
      phaseManager.unshiftPhase(new CommonAnimPhase(pokemon.getBattlerIndex(), undefined, CommonAnim.CURSE)); // TODO: Update animation type
...
export class EncoreTag extends MoveRestrictionBattlerTag {
  ...
  override onAdd(pokemon: Pokemon): void {
    ...
    queueMessage(i18next.t("battlerTags:encoreOnAdd", { pokemonNameWithAffix: getPokemonNameWithAffix(pokemon) }));

    const movePhase = phaseManager.findPhase((m) => m instanceof MovePhase && m.pokemon === pokemon);
    if (movePhase) {
      const movesetMove = pokemon.getMoveset().find((m) => m.moveId === this.moveId);
      if (movesetMove) {
        const lastMove = pokemon.getLastXMoves(1)[0];
        phaseManager.tryReplacePhase(
  ...
  override onRemove(pokemon: Pokemon): void {
    ...
    queueMessage(i18next.t("battlerTags:encoreOnRemove", { pokemonNameWithAffix: getPokemonNameWithAffix(pokemon) }));
  }
}
...
export class OctolockTag extends TrappedTag {
  ...
  override lapse(pokemon: Pokemon, lapseType: BattlerTagLapseType): boolean {
    ...
    if (shouldLapse) {
      phaseManager.unshiftPhase(new StatStageChangePhase(pokemon.getBattlerIndex(), false, [Stat.DEF, Stat.SPDEF], -1));
      return true;
    }
...
export class AquaRingTag extends BattlerTag {
  ...
  override onAdd(pokemon: Pokemon): void {
    ...
    queueMessage(i18next.t("battlerTags:aquaRingOnAdd", { pokemonNameWithAffix: getPokemonNameWithAffix(pokemon) }));
  }
...
export class DrowsyTag extends BattlerTag {
  ...
  override onAdd(pokemon: Pokemon): void {
    ...
    queueMessage(i18next.t("battlerTags:drowsyOnAdd", { pokemonNameWithAffix: getPokemonNameWithAffix(pokemon) }));
  }
...
export abstract class DamagingTrapTag extends TrappedTag {
  ...
  override lapse(pokemon: Pokemon, lapseType: BattlerTagLapseType): boolean {
    ...
    if (ret) {
      ...
      phaseManager.unshiftPhase(new CommonAnimPhase(pokemon.getBattlerIndex(), undefined, this.commonAnim));
...
export class ProtectedTag extends BattlerTag {
  ...
  override onAdd(pokemon: Pokemon): void {
    ...
    queueMessage(i18next.t("battlerTags:protectedOnAdd", { pokemonNameWithAffix: getPokemonNameWithAffix(pokemon) }));
  }

  override lapse(pokemon: Pokemon, lapseType: BattlerTagLapseType): boolean {
    if (lapseType === BattlerTagLapseType.CUSTOM) {
      new CommonBattleAnim(CommonAnim.PROTECT, pokemon).play();
      queueMessage(i18next.t("battlerTags:protectedLapse", { pokemonNameWithAffix: getPokemonNameWithAffix(pokemon) }));
...
export class EnduringTag extends BattlerTag {
  ...
  override onAdd(pokemon: Pokemon): void {
    ...
    queueMessage(i18next.t("battlerTags:enduringOnAdd", { pokemonNameWithAffix: getPokemonNameWithAffix(pokemon) }));
  }

  override lapse(pokemon: Pokemon, lapseType: BattlerTagLapseType): boolean {
    if (lapseType === BattlerTagLapseType.CUSTOM) {
      queueMessage(i18next.t("battlerTags:enduringLapse", { pokemonNameWithAffix: getPokemonNameWithAffix(pokemon) }));
      return true;
    }
...
export class SturdyTag extends BattlerTag {
  ...
  override lapse(pokemon: Pokemon, lapseType: BattlerTagLapseType): boolean {
    if (lapseType === BattlerTagLapseType.CUSTOM) {
      queueMessage(i18next.t("battlerTags:sturdyLapse", { pokemonNameWithAffix: getPokemonNameWithAffix(pokemon) }));
      return true;
    }
...
export class TruantTag extends AbilityBattlerTag {
  ...
  override lapse(pokemon: Pokemon, lapseType: BattlerTagLapseType): boolean {
    ...
    if (lastMove && lastMove.move !== Moves.NONE) {
      ...
      queueMessage(i18next.t("battlerTags:truantLapse", { pokemonNameWithAffix: getPokemonNameWithAffix(pokemon) }));
    }
...
export class CritBoostTag extends BattlerTag {
  ...
  override onAdd(pokemon: Pokemon): void {
    ...
    queueMessage(i18next.t("battlerTags:critBoostOnAdd", { pokemonNameWithAffix: getPokemonNameWithAffix(pokemon) }));
  }
...
export class SaltCuredTag extends BattlerTag {
  ...
  override onAdd(pokemon: Pokemon): void {
    ...
    queueMessage(i18next.t("battlerTags:saltCuredOnAdd", { pokemonNameWithAffix: getPokemonNameWithAffix(pokemon) }));
    this.sourceIndex = globalScene.getPokemonById(this.sourceId!)!.getBattlerIndex(); // TODO: are those bangs correct?
  }
...
export class CursedTag extends BattlerTag {
  ...
  override lapse(pokemon: Pokemon, lapseType: BattlerTagLapseType): boolean {
    ...
    if (ret) {
      ...
      if (!cancelled.value) {
        pokemon.damageAndUpdate(toDmgValue(pokemon.getMaxHp() / 4));
        queueMessage(i18next.t("battlerTags:cursedLapse", { pokemonNameWithAffix: getPokemonNameWithAffix(pokemon) }));
      }
    }
...
export class GulpMissileTag extends BattlerTag {
  ...
  override lapse(pokemon: Pokemon, _lapseType: BattlerTagLapseType): boolean {
    ...
    const moveEffectPhase = phaseManager.getCurrentPhase();
    if (moveEffectPhase instanceof MoveEffectPhase) {
      ...
      if (this.tagType === BattlerTagType.GULP_MISSILE_ARROKUDA) {
        phaseManager.unshiftPhase(new StatStageChangePhase(attacker.getBattlerIndex(), false, [Stat.DEF], -1));
      } else {
...
export class TarShotTag extends BattlerTag {
  ...
  override onAdd(pokemon: Pokemon): void {
    queueMessage(i18next.t("battlerTags:tarShotOnAdd", { pokemonNameWithAffix: getPokemonNameWithAffix(pokemon) }));
  }
}
...
export class ElectrifiedTag extends BattlerTag {
  ...
  override onAdd(pokemon: Pokemon): void {
    ...
    queueMessage(i18next.t("battlerTags:electrifiedOnAdd", { pokemonNameWithAffix: getPokemonNameWithAffix(pokemon) }));
  }
}
...
export class TauntTag extends MoveRestrictionBattlerTag {
  ...
  override onAdd(pokemon: Pokemon) {
    super.onAdd(pokemon);
    queueMessage(i18next.t("battlerTags:tauntOnAdd", { pokemonNameWithAffix: getPokemonNameWithAffix(pokemon) }), 1500);
  }
...
export class SyrupBombTag extends BattlerTag {
  ...
  override onAdd(pokemon: Pokemon) {
    super.onAdd(pokemon);
    queueMessage(i18next.t("battlerTags:syrupBombOnAdd", { pokemonNameWithAffix: getPokemonNameWithAffix(pokemon) }));
  }
  ...
  override lapse(pokemon: Pokemon, _lapseType: BattlerTagLapseType): boolean {
    ...
    queueMessage(i18next.t("battlerTags:syrupBombLapse", { pokemonNameWithAffix: getPokemonNameWithAffix(pokemon) }));
    phaseManager.unshiftPhase(
...
export class TelekinesisTag extends BattlerTag {
  ...
  override onAdd(pokemon: Pokemon) {
    queueMessage(i18next.t("battlerTags:telekinesisOnAdd", { pokemonNameWithAffix: getPokemonNameWithAffix(pokemon) }));
  }
}
...
export class PowerTrickTag extends BattlerTag {
  ...
  override onAdd(pokemon: Pokemon): void {
    ...
    queueMessage(i18next.t("battlerTags:powerTrickActive", { pokemonNameWithAffix: getPokemonNameWithAffix(pokemon) }));
  }
...
export class GrudgeTag extends BattlerTag {
  ...
  override onAdd(pokemon: Pokemon) {
    super.onAdd(pokemon);
    queueMessage(i18next.t("battlerTags:grudgeOnAdd", { pokemonNameWithAffix: getPokemonNameWithAffix(pokemon) }));
  }
