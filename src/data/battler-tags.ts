  ...
  lapse(pokemon: Pokemon, lapseType: BattlerTagLapseType): boolean {
    if (lapseType === BattlerTagLapseType.AFTER_HIT) {
      ...
      if (phaseData?.move.category === MoveCategory.PHYSICAL && pokemon.isOpponent(phaseData.attacker)) {
        ...
        const firstMovePhaseIndex = pokemon.scene.phaseQueue.findIndex((phase) => phase instanceof MovePhase);
...
export class ConfusedTag extends BattlerTag {
  ...
  onAdd(pokemon: Pokemon): void {
    ...
    pokemon.scene.unshiftPhase(
      new CommonAnimPhase(pokemon.scene, pokemon.getBattlerIndex(), undefined, CommonAnim.CONFUSION),
    );
    pokemon.scene.queueMessage(
...
export class InfatuatedTag extends BattlerTag {
  ...
  lapse(pokemon: Pokemon, lapseType: BattlerTagLapseType): boolean {
    ...
    if (ret) {
      ...
      pokemon.scene.unshiftPhase(
        new CommonAnimPhase(pokemon.scene, pokemon.getBattlerIndex(), undefined, CommonAnim.ATTRACT),
      );
    ...
    if (lapseType === BattlerTagLapseType.PRE_MOVE) {
      ...
      if (movePhase instanceof MovePhase) {
        ...
        if (
          ...
          && !(weather && weather.weatherType === WeatherType.HEAVY_RAIN && !weather.isEffectSuppressed(pokemon.scene))
        ) {
          movePhase.fail();
          movePhase.showMoveText();

          pokemon.scene.unshiftPhase(
            new CommonAnimPhase(pokemon.scene, pokemon.getBattlerIndex(), pokemon.getBattlerIndex(), CommonAnim.POWDER),
          );
...
export class NightmareTag extends BattlerTag {
  ...
  lapse(pokemon: Pokemon, lapseType: BattlerTagLapseType): boolean {
    ...
    if (ret) {
      ...
      pokemon.scene.unshiftPhase(
        new CommonAnimPhase(pokemon.scene, pokemon.getBattlerIndex(), undefined, CommonAnim.CURSE),
...
export class OctolockTag extends TrappedTag {
  ...
  lapse(pokemon: Pokemon, lapseType: BattlerTagLapseType): boolean {
    ...
    if (shouldLapse) {
      pokemon.scene.unshiftPhase(
        new StatStageChangePhase(pokemon.scene, pokemon.getBattlerIndex(), false, [Stat.DEF, Stat.SPDEF], -1),
      );
      return true;
    }
...
export abstract class DamagingTrapTag extends TrappedTag {
  ...
  lapse(pokemon: Pokemon, lapseType: BattlerTagLapseType): boolean {
    ...
    if (ret) {
      ...
      pokemon.scene.unshiftPhase(
        new CommonAnimPhase(pokemon.scene, pokemon.getBattlerIndex(), undefined, this.commonAnim),
      );
  ...
  lapse(pokemon: Pokemon, lapseType: BattlerTagLapseType): boolean {
    ...
    if (lapseType === BattlerTagLapseType.CUSTOM) {
      ...
      if (effectPhase instanceof MoveEffectPhase && effectPhase.move.getMove().hasFlag(MoveFlags.MAKES_CONTACT)) {
        const attacker = effectPhase.getPokemon();
        pokemon.scene.unshiftPhase(
          new StatStageChangePhase(pokemon.scene, attacker.getBattlerIndex(), false, [this.stat], this.levels),
        );
      }
    }
...
export class SaltCuredTag extends BattlerTag {
  ...
  lapse(pokemon: Pokemon, lapseType: BattlerTagLapseType): boolean {
    ...
    if (ret) {
      pokemon.scene.unshiftPhase(
        new CommonAnimPhase(pokemon.scene, pokemon.getBattlerIndex(), pokemon.getBattlerIndex(), CommonAnim.SALT_CURE),
      );
  ...
  override lapse(pokemon: Pokemon, lapseType: BattlerTagLapseType): boolean {
    ...
    if (moveEffectPhase instanceof MoveEffectPhase) {
      ...
      if (this.tagType === BattlerTagType.GULP_MISSILE_ARROKUDA) {
        pokemon.scene.unshiftPhase(
          new StatStageChangePhase(pokemon.scene, attacker.getBattlerIndex(), false, [Stat.DEF], -1),
        );
      } else {
...
export class SyrupBombTag extends BattlerTag {
  ...
  override lapse(pokemon: Pokemon, _lapseType: BattlerTagLapseType): boolean {
    ...
    pokemon.scene.unshiftPhase(
      new StatStageChangePhase(pokemon.scene, pokemon.getBattlerIndex(), true, [Stat.SPD], -1, true, false, true),
    );
    return --this.turnCount > 0;
  }
}
