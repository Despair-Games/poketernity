...
export class BlockRecoilDamageAttr extends AbAttr {
  apply(_pokemon: Pokemon, _passive: boolean, _simulated: boolean, cancelled: BooleanHolder, _args: any[]): boolean {
...
export class DoubleBattleChanceAbAttr extends AbAttr {
   ...
   */
  apply(_pokemon: Pokemon, _passive: boolean, _simulated: boolean, _cancelled: BooleanHolder, args: any[]): boolean {
    ...
    if (!simulated) {
      ...
      for (const statStageChangePhase of statStageChangePhases) {
        if (!this.selfTarget && !statStageChangePhase.getPokemon()?.summonData) {
          phaseManager.pushPhase(statStageChangePhase);
        } else {
...
class TypeImmunityStatStageChangeAbAttr extends TypeImmunityAbAttr {
  ...
  applyPreDefend(
    ...
    if (ret) {
      cancelled.value = true; // Suppresses "No Effect" message
      if (!simulated) {
        phaseManager.unshiftPhase(new StatStageChangePhase(pokemon.getBattlerIndex(), true, [this.stat], this.stages));
      }
    }
    ...
    if (this.condition(pokemon, attacker, move) && !move.hitsSubstitute(attacker, pokemon)) {
      ...
      if (this.allOthers) {
        ...
        for (const other of otherPokemon) {
          phaseManager.unshiftPhase(new StatStageChangePhase(other.getBattlerIndex(), false, [this.stat], this.stages));
        }
        return true;
      }
      phaseManager.unshiftPhase(
...
class PostVictoryStatStageChangeAbAttr extends PostVictoryAbAttr {
  ...
  applyPostVictory(pokemon: Pokemon, _passive: boolean, simulated: boolean, _args: any[]): boolean | Promise<boolean> {
    const stat = typeof this.stat === "function" ? this.stat(pokemon) : this.stat;
    if (!simulated) {
      phaseManager.unshiftPhase(new StatStageChangePhase(pokemon.getBattlerIndex(), true, [stat], this.stages));
    }
    return true;
  }
}
...
export class IgnoreOpponentStatStagesAbAttr extends AbAttr {
   ...
   */
  apply(_pokemon: Pokemon, _passive: boolean, _simulated: boolean, _cancelled: BooleanHolder, args: any[]) {
...
export class PostIntimidateStatStageChangeAbAttr extends AbAttr {
  ...
  apply(pokemon: Pokemon, _passive: boolean, simulated: boolean, cancelled: BooleanHolder, _args: any[]): boolean {
    if (!simulated) {
      phaseManager.pushPhase(new StatStageChangePhase(pokemon.getBattlerIndex(), false, this.stats, this.stages));
    }
...
export class PostSummonAbAttr extends AbAttr {
   ...
   * @returns true if application of the ability succeeds
   */
  applyPostSummon(_pokemon: Pokemon, _passive: boolean, _simulated: boolean, _args: any[]): boolean | Promise<boolean> {
    return false;
  }
}
...
export class PostSummonRemoveArenaTagAbAttr extends PostSummonAbAttr {
  ...
  applyPostSummon(_pokemon: Pokemon, _passive: boolean, simulated: boolean, _args: any[]): boolean | Promise<boolean> {
    if (!simulated) {
...
export class PostSummonMessageAbAttr extends PostSummonAbAttr {
  ...
  applyPostSummon(pokemon: Pokemon, _passive: boolean, simulated: boolean, _args: any[]): boolean {
    if (!simulated) {
      queueMessage(this.messageFunc(pokemon));
    }
...
export class PostSummonUnnamedMessageAbAttr extends PostSummonAbAttr {
  ...
  applyPostSummon(_pokemon: Pokemon, _passive: boolean, simulated: boolean, _args: any[]): boolean {
    if (!simulated) {
...
export class PostSummonStatStageChangeAbAttr extends PostSummonAbAttr {
  ...
  applyPostSummon(pokemon: Pokemon, passive: boolean, simulated: boolean, _args: any[]): boolean {
    if (simulated) {
      return true;
    }
    ...
    if (this.selfTarget) {
      ...
      phaseManager.unshiftPhase(new StatStageChangePhase(pokemon.getBattlerIndex(), true, this.stats, this.stages));
      return true;
    }
    for (const opponent of pokemon.getOpponents()) {
      ...
      if (!cancelled.value) {
        phaseManager.unshiftPhase(new StatStageChangePhase(opponent.getBattlerIndex(), false, this.stats, this.stages));
      }
    }
    return true;
  }
...
export class DownloadAbAttr extends PostSummonAbAttr {
  ...
  applyPostSummon(pokemon: Pokemon, _passive: boolean, simulated: boolean, _args: any[]): boolean {
    ...
    if (this.enemyDef > 0 && this.enemySpDef > 0) {
      ...
      if (!simulated) {
        phaseManager.unshiftPhase(new StatStageChangePhase(pokemon.getBattlerIndex(), false, this.stats, 1));
      }
      return true;
    }
...
export class PostSummonUserFieldRemoveStatusEffectAbAttr extends PostSummonAbAttr {
   ...
   */
  applyPostSummon(pokemon: Pokemon, _passive: boolean, simulated: boolean, _args: any[]): boolean | Promise<boolean> {
    ...
    if (!simulated) {
      for (const pokemon of allowedParty) {
        if (pokemon.status && this.statusEffect.includes(pokemon.status.effect)) {
          queueMessage(getStatusEffectHealText(pokemon.status.effect, getPokemonNameWithAffix(pokemon)));
...
export class PostSummonWeatherSuppressedFormChangeAbAttr extends PostSummonAbAttr {
   ...
   */
  applyPostSummon(_pokemon: Pokemon, _passive: boolean, simulated: boolean, _args: any[]) {
  ...
  override apply(pokemon: Pokemon, _passive: boolean, simulated: boolean, _cancelled: null, _args: any[]): boolean {
    ...
    if (globalScene.currentBattle?.double && pokemon.getAlly()?.species.speciesId === Species.DONDOZO) {
      ...
      if (!simulated) {
        ...
        phaseManager.tryRemovePhase((phase) => phase instanceof MovePhase && phase.pokemon === pokemon);
      }
      return true;
    }
    return false;
...
export class PreSwitchOutResetStatusAbAttr extends PreSwitchOutAbAttr {
  applyPreSwitchOut(pokemon: Pokemon, _passive: boolean, simulated: boolean, _args: any[]): boolean | Promise<boolean> {
    if (pokemon.status) {
      if (!simulated) {
        pokemon.resetStatus();
        pokemon.updateInfo();
...
export class ConditionalCritAbAttr extends AbAttr {
   ...
   */
  apply(pokemon: Pokemon, _passive: boolean, _simulated: boolean, _cancelled: BooleanHolder, args: any[]): boolean {
...
export class BlockStatusDamageAbAttr extends AbAttr {
   ...
   */
  apply(pokemon: Pokemon, _passive: boolean, _simulated: boolean, cancelled: BooleanHolder, _args: any[]): boolean {
...
export class PostTurnAbAttr extends AbAttr {
  applyPostTurn(_pokemon: Pokemon, _passive: boolean, _simulated: boolean, _args: any[]): boolean | Promise<boolean> {
    return false;
  }
}
...
export class PostTurnStatusHealAbAttr extends PostTurnAbAttr {
   ...
   */
  applyPostTurn(pokemon: Pokemon, passive: boolean, simulated: boolean, _args: any[]): boolean | Promise<boolean> {
    if (pokemon.status && this.effects.includes(pokemon.status.effect)) {
      if (!pokemon.isFullHp()) {
        if (!simulated) {
          const abilityName = (!passive ? pokemon.getAbility() : pokemon.getPassiveAbility()).name;
...
export class PostTurnResetStatusAbAttr extends PostTurnAbAttr {
  ...
  applyPostTurn(pokemon: Pokemon, _passive: boolean, simulated: boolean, _args: any[]): boolean {
    if (this.allyTarget) {
    ...
    if (this.target?.status) {
      if (!simulated) {
        queueMessage(getStatusEffectHealText(this.target.status?.effect, getPokemonNameWithAffix(this.target)));
  ...
  applyPostTurn(pokemon: Pokemon, _passive: boolean, simulated: boolean, _args: any[]): boolean {
    ...
    if (!simulated) {
      if (canRaise.length > 0) {
        ...
        phaseManager.unshiftPhase(new StatStageChangePhase(pokemon.getBattlerIndex(), true, [raisedStat], 2));
      }
      if (canLower.length > 0) {
        const loweredStat = canLower[pokemon.randSeedInt(canLower.length)];
        phaseManager.unshiftPhase(new StatStageChangePhase(pokemon.getBattlerIndex(), true, [loweredStat], -1));
      }
    }
  ...
  applyPostTurn(pokemon: Pokemon, _passive: boolean, simulated: boolean, _args: any[]): boolean {
    if (!simulated) {
      if (!pokemon.turnData.switchedInThisTurn && !pokemon.turnData.failedRunAway) {
        phaseManager.unshiftPhase(new StatStageChangePhase(pokemon.getBattlerIndex(), true, [Stat.SPD], 1));
      } else {
        return false;
      }
    }
...
export class PostTurnHealAbAttr extends PostTurnAbAttr {
  applyPostTurn(pokemon: Pokemon, passive: boolean, simulated: boolean, _args: any[]): boolean {
    if (!pokemon.isFullHp()) {
      if (!simulated) {
        const abilityName = (!passive ? pokemon.getAbility() : pokemon.getPassiveAbility()).name;
        phaseManager.unshiftPhase(
...
export class PostTurnHurtIfSleepingAbAttr extends PostTurnAbAttr {
   ...
   */
  applyPostTurn(pokemon: Pokemon, _passive: boolean, simulated: boolean, _args: any[]): boolean | Promise<boolean> {
    ...
    for (const opp of pokemon.getOpponents()) {
      if (
        ...
        if (!simulated) {
          ...
          queueMessage(i18next.t("abilityTriggers:badDreams", { pokemonName: getPokemonNameWithAffix(opp) }));
        }
        hadEffect = true;
      }
    }
...
export class PostBiomeChangeWeatherChangeAbAttr extends PostBiomeChangeAbAttr {
  ...
  apply(_pokemon: Pokemon, _passive: boolean, simulated: boolean, _cancelled: BooleanHolder, _args: any[]): boolean {
    if (!globalScene.arena.weather?.isImmutable()) {
      if (simulated) {
        return globalScene.arena.weather?.weatherType !== this.weatherType;
      } else {
    ...
    if (
      ...
      if (!simulated) {
        ...
        if (move.getMove() instanceof AttackMove || move.getMove() instanceof StatusMove) {
          ...
          phaseManager.unshiftPhase(new MovePhase(dancer, target, move, true, true));
        } else if (move.getMove() instanceof SelfStatusMove) {
          // If the move is a SelfStatusMove (ie. Swords Dance) the Dancer should replicate it on itself
          phaseManager.unshiftPhase(new MovePhase(dancer, [dancer.getBattlerIndex()], move, true, true));
        }
      }
      return true;
    }
...
export class PostItemLostAbAttr extends AbAttr {
  applyPostItemLost(_pokemon: Pokemon, _simulated: boolean, _args: any[]): boolean | Promise<boolean> {
    return false;
  }
}
...
export class PostItemLostApplyBattlerTagAbAttr extends PostItemLostAbAttr {
   ...
   */
  applyPostItemLost(pokemon: Pokemon, simulated: boolean, _args: any[]): boolean | Promise<boolean> {
    if (!pokemon.getTag(this.tagType) && !simulated) {
...
export class HealFromBerryUseAbAttr extends AbAttr {
  ...
  apply(pokemon: Pokemon, passive: boolean, simulated: boolean, ..._args: [BooleanHolder, any[]]): boolean {
...
export class PostBattleLootAbAttr extends PostBattleAbAttr {
   ...
   */
  applyPostBattle(pokemon: Pokemon, _passive: boolean, simulated: boolean, args: any[]): boolean {
...
export class FlinchStatStageChangeAbAttr extends FlinchEffectAbAttr {
  ...
  apply(pokemon: Pokemon, _passive: boolean, simulated: boolean, _cancelled: BooleanHolder, _args: any[]): boolean {
    if (!simulated) {
      phaseManager.unshiftPhase(new StatStageChangePhase(pokemon.getBattlerIndex(), true, this.stats, this.stages));
    }
    return true;
...
export class MoveAbilityBypassAbAttr extends AbAttr {
  ...
  apply(pokemon: Pokemon, _passive: boolean, _simulated: boolean, cancelled: BooleanHolder, args: any[]): boolean {
...
export class SuppressFieldAbilitiesAbAttr extends AbAttr {
  ...
  apply(_pokemon: Pokemon, _passive: boolean, _simulated: boolean, cancelled: BooleanHolder, args: any[]): boolean {
...
export class MoneyAbAttr extends PostBattleAbAttr {
   ...
   * @returns `true` if successful
   */
  applyPostBattle(_pokemon: Pokemon, _passive: boolean, simulated: boolean, args: any[]): boolean {
    if (!simulated && args[0]) {
...
export class BypassSpeedChanceAbAttr extends AbAttr {
   ...
   */
  apply(pokemon: Pokemon, _passive: boolean, simulated: boolean, _cancelled: BooleanHolder, args: any[]): boolean {
    if (simulated) {
      return false;
    }
...
export function applyAbAttrs(
  ...
  return applyAbAttrsInternal<AbAttr>(
    attrType,
    pokemon,
...
export function applyPostBattleInitAbAttrs(
  ...
  return applyAbAttrsInternal<PostBattleInitAbAttr>(
    attrType,
    pokemon,
...
export function applyPreDefendAbAttrs(
  ...
  return applyAbAttrsInternal<PreDefendAbAttr>(
    attrType,
    pokemon,
...
export function applyPostMoveUsedAbAttrs(
  ...
  return applyAbAttrsInternal<PostMoveUsedAbAttr>(
    attrType,
    pokemon,
...
export function applyStatMultiplierAbAttrs(
  ...
  return applyAbAttrsInternal<StatMultiplierAbAttr>(
    attrType,
    pokemon,
...
export function applyPostDamageAbAttrs(
  ...
  return applyAbAttrsInternal<PostDamageAbAttr>(
    attrType,
    pokemon,
...
export function applyPreAttackAbAttrs(
  ...
  return applyAbAttrsInternal<PreAttackAbAttr>(
    attrType,
    pokemon,
...
export function applyPostAttackAbAttrs(
  ...
  return applyAbAttrsInternal<PostAttackAbAttr>(
    attrType,
    pokemon,
...
export function applyPostKnockOutAbAttrs(
  ...
  return applyAbAttrsInternal<PostKnockOutAbAttr>(
    attrType,
    pokemon,
...
export function applyPostVictoryAbAttrs(
  ...
  return applyAbAttrsInternal<PostVictoryAbAttr>(
    attrType,
    pokemon,
...
export function applyPostSummonAbAttrs(
  ...
  return applyAbAttrsInternal<PostSummonAbAttr>(
    attrType,
    pokemon,
...
export function applyPreSwitchOutAbAttrs(
  ...
  return applyAbAttrsInternal<PreSwitchOutAbAttr>(
    attrType,
    pokemon,
...
export function applyPreStatStageChangeAbAttrs(
  ...
  return applyAbAttrsInternal<PreStatStageChangeAbAttr>(
    attrType,
    pokemon,
...
export function applyPostStatStageChangeAbAttrs(
  ...
  return applyAbAttrsInternal<PostStatStageChangeAbAttr>(
    attrType,
    pokemon,
...
export function applyPreSetStatusAbAttrs(
  ...
  return applyAbAttrsInternal<PreSetStatusAbAttr>(
    attrType,
    pokemon,
...
export function applyPreApplyBattlerTagAbAttrs(
  ...
  return applyAbAttrsInternal<PreApplyBattlerTagAbAttr>(
    attrType,
    pokemon,
...
export function applyPreWeatherEffectAbAttrs(
  ...
  return applyAbAttrsInternal<PreWeatherDamageAbAttr>(
    attrType,
    pokemon,
...
export function applyPostTurnAbAttrs(
  ...
  return applyAbAttrsInternal<PostTurnAbAttr>(
    attrType,
    pokemon,
...
export function applyPostWeatherChangeAbAttrs(
  ...
  return applyAbAttrsInternal<PostWeatherChangeAbAttr>(
    attrType,
    pokemon,
...
export function applyPostWeatherLapseAbAttrs(
  ...
  return applyAbAttrsInternal<PostWeatherLapseAbAttr>(
    attrType,
    pokemon,
...
export function applyPostTerrainChangeAbAttrs(
  ...
  return applyAbAttrsInternal<PostTerrainChangeAbAttr>(
    attrType,
    pokemon,
...
export function applyCheckTrappedAbAttrs(
  ...
  return applyAbAttrsInternal<CheckTrappedAbAttr>(
    attrType,
    pokemon,
...
export function applyPostBattleAbAttrs(
  ...
  return applyAbAttrsInternal<PostBattleAbAttr>(
    attrType,
    pokemon,
...
export function applyPostFaintAbAttrs(
  ...
  return applyAbAttrsInternal<PostFaintAbAttr>(
    attrType,
    pokemon,
...
export function applyPostItemLostAbAttrs(
  ...
  return applyAbAttrsInternal<PostItemLostAbAttr>(
    attrType,
    pokemon,
...
function queueShowAbility(pokemon: Pokemon, passive: boolean): void {
  phaseManager.unshiftPhase(new ShowAbilityPhase(pokemon.id, passive));
  phaseManager.clearPhaseQueueSplice();
}
    ...
    new Ability(Abilities.ANALYTIC, 5).attr(
      ...
      (user, _target, _move) => {
        const movePhase = phaseManager.findPhase(
          (phase) => phase instanceof MovePhase && phase.pokemon.id !== user?.id,
        );
