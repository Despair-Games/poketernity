    ...
    if (
      ...
      if (!simulated) {
        if (move.getMove().isSelfStatusMove()) {
          ...
          globalScene.useMove({ pokemon, targets: [pokemon.getBattlerIndex()], move, followUp: true, when: "eager" });
        } else {
          ...
          globalScene.useMove({ pokemon, targets: target, move, followUp: true, when: "eager" });
        }
      }
      return true;
    }
