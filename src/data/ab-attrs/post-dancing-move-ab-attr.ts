    ...
    if (
      ...
      if (!simulated) {
        if (move.getMove().isSelfStatusMove()) {
          ...
          globalScene.useMove({ pokemon, targets: [pokemon.getBattlerIndex()], move, followUp: true, eager: true });
        } else {
          ...
          globalScene.useMove({ pokemon, targets: target, move, followUp: true, eager: true });
        }
      }
      return true;
    }
