...
export class PostDefendApplyBattlerTagAbAttr extends PostDefendAbAttr {
  ...
  public override apply({ pokemon, simulated, move }: PostDefendAbAttrParams): void {
    if (simulated) {
      return;
    }
    ...
    // TODO: shouldn't this be in `getTriggerMessage()`?
    // TODO: this `AbAttr` is used by both Wind Power and Electromorphosis, doesn't that make this extra incorrect?
    const pokemonName = getPokemonNameWithAffix(pokemon);
    const moveName = pokemon.getPokemonMove(move.id)?.name ?? move.name;
    globalScene.phaseManager.createAndUnshiftPhase(
      "MessagePhase",
      i18next.t("abilityTriggers:windPowerCharged", { pokemonName, moveName }),
    );
  }
