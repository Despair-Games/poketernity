...
export class StarterSelectUiHandler extends MessageUiHandler {
  ...
  private statsContainer: IVGraph;
  ...
  protected currentMode: StarterSelectMode = StarterSelectMode.NONE;
  ...
  protected override setup() {
    ...
    this.statsContainer = new IVGraph(6, 16);
  ...
  public override processInput(button: Button): boolean {
    ...
    if (button === Button.SUBMIT) {
      ...
      if (this.currentMode === StarterSelectMode.FILTER && this.filterBar.openDropDown) {
    ...
    } else if (this.currentMode === StarterSelectMode.FILTER) {
      switch (button) {
        case Button.LEFT:
        ...
        case Button.UP:
          if (this.filterBar.openDropDown) {
            ...
            if (numberOfStarters % 9 > targetCol) {
              newCursor = numberOfStarters - (numberOfStarters % 9) + targetCol;
            } else {
      ...
      let starterContainer: StarterContainer | undefined;
        ...
        switch (button) {
          ...
          case Button.CYCLE_FORM:
            if (this.canCycleForm) {
              ...
              starterAttributes.teraType = this.lastSpecies.forms[newFormIndex].type1;
  ...
  addToParty(
    ...
    if (this.speciesLoaded.get(species.speciesId)) {
      getPokemonSpeciesForm(species.speciesId, formIndex).cry();
    }
    this.updateInstructions();
  }
  ...
  updateStarters = () => {
    ...
    for (const currentFilteredContainer of this.validStarterContainers) {
      ...
      starterSprite.setTexture(
        currentFilteredContainer.species.getIconAtlasKey(formIndex, shiny, variant),
        currentFilteredContainer.species.getIconId(female, formIndex, shiny, variant),
      );
  ...
  /**
   * Change the current mode for Starter Selection, optionally updating the cursor(s).
   * Hides graphical elements exclusive to the previous mode, and shows elements for the new mode.
   * @param newMode - the {@linkcode StarterSelectMode} to set.
   * @param cursor - Optional. The value to set the cursor to, if applicable
   * @param scrollCursor - Optional. The value to set the scrollingcursor to, if applicable
   * @returns `true` if the mode, was changed, `false` otherwise.
   */
  private setMode(newMode: StarterSelectMode, cursor?: number, scrollCursor?: number): boolean {
    ...
    switch (newMode) {
      case StarterSelectMode.STARTER_GRID:
        ...
        break;
      case StarterSelectMode.FILTER:
        this.filterBar.cursorObj.setVisible(true);
        this.filterBarCursor = cursor ?? 0;
        this.setCursor(this.filterBarCursor);
        break;
      case StarterSelectMode.PARTY:
        this.starterIconsCursorIndex = cursor ?? 0;
        this.moveStarterIconsCursor(this.starterIconsCursorIndex);
        break;
      case StarterSelectMode.START:
        this.startCursorObj.setVisible(true);
        break;
      case StarterSelectMode.NONE:
        this.hideInstructions();
        this.cursor = -1;
        break;
    }
  ...
  setSpecies(species: PokemonSpecies | null) {
    ...
    if (this.lastSpecies) {
      ...
      this.checkIconId(lastSpeciesIcon, this.lastSpecies, female, formIndex, shiny, variant);
    ...
    if (species && (this.speciesStarterDexEntry?.seenAttr || this.speciesStarterDexEntry?.caughtAttr)) {
      ...
      if (this.speciesStarterDexEntry?.caughtAttr) {
        ...
        //Growth translate
        let growthReadable = toReadableString(enumValueToKey(GrowthRate, species.growthRate));
        ...
        if (starterIndex > -1) {
          ...
          this.setSpeciesDetails(species, {
            ...
            female: props.gender === Gender.FEMALE,
  ...
  setSpeciesDetails(species: PokemonSpecies, options: SpeciesDetails = {}): void {
    ...
    this.natureCursor = -1;
    this.teraCursor = ElementalType.UNKNOWN;
    ...
    if (species) {
      ...
      if (female) {
        this.dexAttrCursor |= DexAttr.FEMALE;
      } else if (species.malePercent !== null) {
    ...
    if (species) {
      ...
      if (!dexEntry.caughtAttr) {
        ...
        if (female === undefined || female !== (props.gender === Gender.FEMALE)) {
          female = props.gender === Gender.FEMALE;
        }
  ...
  popStarter(index: number): void {
    ...
    for (let s = 0; s < this.starterSpecies.length; s++) {
      this.updatePartyIcon(this.starterSpecies[s], s);
      if (s >= index) {
    ...
    this.starterIcons[this.starterSpecies.length].setTexture("pokemon_icons_0", "unknown");
    ...
    } else if (this.currentMode === StarterSelectMode.START && this.starterSpecies.length === 0) {
  ...
  protected override clear(): void {
    ...
    this.setMode(StarterSelectMode.NONE); // Back to no mode
    this.activeTooltip = undefined;
    globalScene.ui.hideTooltip();
