...
export default class SummaryUiHandler extends UiHandler {
  ...
  setup() {
    ...
    this.pokemonSprite = this.scene.initPokemonSprite(
      this.scene.add.sprite(56, -106, "pkmn__sub"),
      undefined,
      false,
      true,
    );
    ...
    const moveEffectLabels = addTextObject(
      this.scene,
      8,
      12,
  ...
  show(args: any[]): boolean {
    ...
    if (
      this.scene.gameData.starterData[this.pokemon.species.getRootSpeciesId()].classicWinCount > 0
      && this.scene.gameData.starterData[this.pokemon.species.getRootSpeciesId(true)].classicWinCount > 0
    ) {
    ...
    if (this.candyShadow.visible) {
      this.candyShadow.on("pointerover", () =>
        (this.scene as BattleScene).ui.showTooltip("", `${currentFriendship}/${friendshipCap}`, true),
      );
      this.candyShadow.on("pointerout", () => (this.scene as BattleScene).ui.hideTooltip());
    }

    this.candyCountText.setText(
      `x${this.scene.gameData.starterData[this.pokemon.species.getRootSpeciesId()].candyCount}`,
    );
    ...
    if (this.friendshipShadow.visible) {
      this.friendshipShadow.on("pointerover", () =>
        this.scene.ui.showTooltip("", `${i18next.t("pokemonSummary:friendship")}`, true),
      );
    ...
    switch (page) {
      case Page.PROFILE:
        ...
        const trainerText = addBBCodeTextObject(
          ...
          7,
          12,
          `${i18next.t("pokemonSummary:ot")}/${getBBCodeFrag(loggedInUser?.username || i18next.t("pokemonSummary:unknown"), this.scene.gameData.gender === PlayerGender.FEMALE ? TextStyle.SUMMARY_PINK : TextStyle.SUMMARY_BLUE)}`,
        ...
        const trainerIdText = addTextObject(
          ...
          174,
          12,
          this.scene.gameData.trainerId.toString(),
          TextStyle.SUMMARY_ALT,
        );
        trainerIdText.setOrigin(0, 0);
        profileContainer.add(trainerIdText);

        const typeLabel = addTextObject(
          this.scene,
          7,
          28,
        ...
        if (this.pokemon?.hasPassive()) {
          ...
          this.abilityPrompt = this.scene.add.image(
            0,
            0,
            !this.scene.inputController?.gamepadSupport ? "summary_profile_prompt_z" : "summary_profile_prompt_a",
          );
      ...
      case Page.STATS:
        ...
        itemModifiers.forEach((item, i) => {
          ...
          icon.on("pointerover", () =>
            (this.scene as BattleScene).ui.showTooltip(item.type.name, item.type.getDescription(this.scene), true),
          );
        ...
        const nextLvExp =
          pkmLvl < this.scene.getMaxExpLevel() ? getLevelTotalExp(pkmLvl + 1, pkmSpeciesGrowthRate) - pkmExp : 0;
      ...
      case Page.MOVES:
        ...
        this.moveDescriptionText = addTextObject(this.scene, 2, 84, "", TextStyle.WINDOW_ALT, {
          wordWrap: { width: 1212 },
        });
