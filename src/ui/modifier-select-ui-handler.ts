...
export default class ModifierSelectUiHandler extends AwaitableUiHandler {
  ...
  show(args: any[]): boolean {
    ...
    const partyHasHeldItem =
      this.player && !!this.scene.findModifiers((m) => m instanceof PokemonHeldItemModifier && m.isTransferable).length;
...
class ModifierOption extends Phaser.GameObjects.Container {
  ...
  setup() {
    ...
    this.itemText = addTextObject(this.scene, 0, 35, this.modifierTypeOption.type?.name!, TextStyle.PARTY, {
      align: "center",
