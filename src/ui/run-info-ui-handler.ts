...
export default class RunInfoUiHandler extends UiHandler {
  ...
  override show(args: any[]): boolean {
    ...
    const gameStatsBg = this.scene.add.rectangle(
      0,
      0,
  ...
  private addHeader() {
    ...
    if (this.runInfo.modifiers.length !== 0) {
      ...
      const abilityButtonText = addTextObject(
        this.scene,
        8,
        0,
      ...
      if (gamepadType === "touch") {
        ...
        abilityButtonElement = new Phaser.GameObjects.Sprite(
          ...
          2,
          gamepadType,
          this.scene.inputController?.getIconForLatestInputRecorded(SettingKeyboard.Button_Cycle_Ability),
        );
      }
  ...
  private async parseRunResult() {
    ...
    if (this.isVictory) {
      ...
      const shinyButtonText = addTextObject(
        this.scene,
        8,
        0,
      ...
      const formButtonText = addTextObject(
        this.scene,
        8,
        12,
      ...
      if (gamepadType === "touch") {
        ...
        shinyButtonElement = new Phaser.GameObjects.Sprite(
          ...
          4,
          gamepadType,
          this.scene.inputController?.getIconForLatestInputRecorded(SettingKeyboard.Button_Cycle_Shiny),
        );
        formButtonElement = new Phaser.GameObjects.Sprite(
          ...
          16,
          gamepadType,
          this.scene.inputController?.getIconForLatestInputRecorded(SettingKeyboard.Button_Cycle_Form),
        );
      }
  ...
  private parseRunStatus() {
    ...
    if (this.runInfo.battleType === BattleType.WILD) {
      ...
      const textBox = addTextObject(this.scene, 0, 0, boxString, TextStyle.WINDOW, {
        fontSize: "35px",
        wordWrap: { width: 200 },
      });
      ...
      const mysteryEncounterTitle = i18next.t(
        this.scene.getMysteryEncounter(this.runInfo.mysteryEncounterType as MysteryEncounterType, true).localizationKey
          + ":title",
      );
      const descContainer = this.scene.add.container(0, 0);
      const textBox = addTextObject(this.scene, 0, 0, mysteryEncounterTitle, TextStyle.WINDOW, {
        fontSize: "45px",
        wordWrap: { width: 160 },
      });
      descContainer.add(textBox);
  ...
  private async parseRunInfo(windowX: number, windowY: number) {
    ...
    if (
      ...
      && this.runInfo.waveIndex === this.scene.gameData.gameStats.highestEndlessWave
    ) {
    ...
    const runInfoText = addBBCodeTextObject(this.scene, 7, 0, "", TextStyle.WINDOW, {
      fontSize: "50px",
    ...
    const luckValue = Phaser.Math.Clamp(
      this.runInfo.party
        .map((p) => p.toPokemon(this.scene).getLuck())
        .reduce((total: integer, value: integer) => (total += value), 0),
      0,
      14,
    );
    ...
    if (this.runInfo.modifiers.length) {
      ...
      const modifierIconsContainer = this.scene.add.container(
        8,
  ...
  private parsePartyInfo(): void {
    ...
    party.forEach((p: PokemonData, i: integer) => {
      ...
      // Japanese is set to a greater line spacing of 35px in addBBCodeTextObject() if lineSpacing < 12.
      const lineSpacing = i18next.resolvedLanguage === "ja" ? 12 : 3;
      const pokeInfoText = addBBCodeTextObject(this.scene, 0, 0, pName, TextStyle.SUMMARY, {
      ...
      // Column 1: HP Atk Def
      const pokeStatText1 = addBBCodeTextObject(this.scene, -5, 0, hp, TextStyle.SUMMARY, {
        fontSize: textContainerFontSize,
        lineSpacing: lineSpacing,
      });
      ...
      const pokeStatText2 = addBBCodeTextObject(this.scene, 25, 0, spatk, TextStyle.SUMMARY, {
        fontSize: textContainerFontSize,
        lineSpacing: lineSpacing,
      });
  ...
  private createVictorySplash(): void {
    ...
    const text = addTextObject(
      ...
      this.scene.game.canvas.height / 6 - 16,
