...
import { ArenaHazardTag, getArenaTag } from "#app/data/arena-tag";
...
export class Arena {
  ...
  addTag(
    ...
    if (existingTag) {
      ...
      if (existingTag instanceof ArenaHazardTag) {
        const { tagType, side, turnCount, layers, maxLayers } = existingTag as ArenaHazardTag;
    ...
    if (newTag) {
      ...
      const { layers = 0, maxLayers = 0 } = newTag instanceof ArenaHazardTag ? newTag : {};
