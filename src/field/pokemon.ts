...
export default abstract class Pokemon extends Phaser.GameObjects.Container {
  ...
  init(): void {
    ...
    const getSprite = (hasShadow?: boolean) => {
      const ret = this.scene.addPokemonSprite(
        this,
        0,
        0,
      ...
      Promise.allSettled(moveIds.map((m) => initMoveAnim(this.scene, m))).then(() => {
        ...
        this.scene.load.once(Phaser.Loader.Events.COMPLETE, () => {
          if (this.isPlayer()) {
            ...
            console.warn = () => {};
            const battleFrameNames = this.scene.anims.generateFrameNames(this.getBattleSpriteKey(), {
              zeroPad: 4,
              suffix: ".png",
              start: 1,
              end: 400,
  ...
  getHeldItems(): PokemonHeldItemModifier[] {
    ...
    return this.scene.findModifiers(
  ...
  public getTeraType(): Type {
    ...
    if (this.scene !== undefined) {
      const teraModifier = this.scene.findModifier(
        (m) => m instanceof TerastallizeModifier && m.pokemonId === this.id && !!m.getBattlesLeft(),
        this.isPlayer(),
  ...
  public isTrapped(trappedAbMessages: string[] = [], simulated: boolean = true): boolean {
    ...
    return (
      ...
      || !!this.scene.arena.getTagOnSide(ArenaTagType.FAIRY_LOCK, side)
    );
  }
  ...
  getAttackTypeEffectiveness(
    ...
    if (
      ...
      && !arena.weather.isEffectSuppressed(this.scene)
  ...
  public generateFusionSpecies(forStarter?: boolean): void {
    ...
    this.fusionSpecies =
      fusionOverride
      ?? this.scene.randomSpecies(this.scene.currentBattle?.waveIndex || 0, this.level, false, filter, true);
    ...
    this.fusionFormIndex = this.scene.getSpeciesFormIndex(
      this.fusionSpecies,
      this.fusionGender,
      this.getNature(),
      true,
  ...
  showInfo(): void {
    if (!this.battleInfo.visible) {
      const otherBattleInfo = this.scene.fieldUI
        .getAll()
        .slice(0, 4)
  ...
  getOpponents(): Pokemon[] {
    return ((this.isPlayer() ? this.scene.getEnemyField() : this.scene.getPlayerField()) as Pokemon[]).filter((p) =>
      p.isActive(),
    );
  }
  ...
  getAttackDamage(
    ...
    const arenaAttackTypeMultiplier = new Utils.NumberHolder(
      this.scene.arena.getAttackTypeMultiplier(moveType, source.isGrounded()),
    );
    ...
    this.scene.arena.applyTagsForSide(
      WeakenMoveScreenTag,
      defendingSide,
      simulated,
      source,
    ...
    const mistyTerrainMultiplier =
      this.scene.arena.terrain?.terrainType === TerrainType.MISTY && this.isGrounded() && moveType === Type.DRAGON
        ? 0.5
        : 1;
    ...
    if (moveCategory.value === MoveCategory.STATUS) {
      ...
      if (this.isFainted()) {
        ...
        this.scene.unshiftPhase(
          new FaintPhase(this.scene, this.getBattlerIndex(), isOneHitKo, destinyTag, grudgeTag, source),
        );
    ...
    let faintCryTimer: Phaser.Time.TimerEvent | null = this.scene.time.addEvent({
      ...
      callback: () => {
        ...
        if (i === transitionIndex && fusionCryKey) {
          ...
          fusionCry = this.scene.playSound(
            fusionCryKey,
          ...
          SoundFade.fadeIn(
            ...
            this.scene.masterVolume * this.scene.fieldVolume,
            0,
          );
        }
        rate *= 0.99;
    ...
    switch (effect) {
      ...
      case StatusEffect.FREEZE:
        if (
          ...
          || (!ignoreField
            && this.scene?.arena?.weather?.weatherType
            && [WeatherType.SUNNY, WeatherType.HARSH_SUN].includes(this.scene.arena.weather.weatherType))
        ) {
          return false;
        }
        break;
      ...
      if (this.shiny && variantColors && variantColors[this.variant]) {
        Object.keys(variantColors[this.variant]).forEach((k) => {
          variantColorSet.set(
            ...
            Array.from(Object.values(Utils.rgbHexToRgba(variantColors[this.variant][k]))),
          );
        });
      }
    ...
    for (let f = 0; f < 2; f++) {
      ...
      if (this.fusionShiny && variantColors && variantColors[this.fusionVariant]) {
        Object.keys(variantColors[this.fusionVariant]).forEach((k) => {
          variantColorSet.set(
            Utils.rgbaToInt(Array.from(Object.values(Utils.rgbHexToRgba(k)))),
            Array.from(Object.values(Utils.rgbHexToRgba(variantColors[this.fusionVariant][k]))),
          );
        });
      }
  ...
  randSeedInt(range: integer, min: integer = 0): integer {
    return this.scene.currentBattle ? this.scene.randBattleSeedInt(range, min) : Utils.randSeedInt(range, min);
  }
...
export class PlayerPokemon extends Pokemon {
  ...
  tryPopulateMoveset(moveset: StarterMoveset): boolean {
    if (
      !this.getSpeciesForm().validateStarterMoveset(
        moveset,
        this.scene.gameData.starterData[this.species.getRootSpeciesId()].eggMoves,
      )
    ) {
      return false;
    }
      ...
      this.scene.ui.setMode(
        ...
        (slotIndex: integer, option: PartyOption) => {
          if (slotIndex >= this.scene.currentBattle.getBattlerCount() && slotIndex < 6) {
            this.scene.prependToPhase(
              new SwitchSummonPhase(this.scene, switchType, this.getFieldIndex(), slotIndex, false),
              MoveEndPhase,
            );
          }
  ...
  addFriendship(friendship: number): void {
    if (friendship > 0) {
      ...
      const starterAmount = new Utils.NumberHolder(
        Math.floor(
          (amount.value * (this.scene.gameMode.isClassic ? candyFriendshipMultiplier : 1))
          ...
          if (slotIndex >= 0 && slotIndex < 6) {
            ...
            if (this.scene.currentBattle.double && this.scene.getPlayerParty().length > 1) {
              ...
              if (slotIndex <= 1) {
                ...
                this.scene.unshiftPhase(
                  new SwitchSummonPhase(this.scene, SwitchType.SWITCH, pokemon.getFieldIndex(), slotIndex, false, true),
                );
  ...
  private handleSpecialEvolutions(evolution: SpeciesFormEvolution) {
    ...
    if (evoSpecies?.speciesId === Species.NINCADA && evolution.speciesId === Species.NINJASK) {
      ...
      if (newEvolution.condition?.predicate(this)) {
        const newPokemon = this.scene.addPlayerPokemon(
          this.species,
          this.level,
          this.abilityIndex,
          this.formIndex,
  ...
  getPossibleForm(formChange: SpeciesFormChange): Promise<Pokemon> {
    return new Promise((resolve) => {
      ...
      const ret = this.scene.addPlayerPokemon(
        this.species,
        this.level,
        this.abilityIndex,
        formIndex,
  ...
  fuse(pokemon: PlayerPokemon): Promise<void> {
    return new Promise((resolve) => {
      ...
      const fusedPartyMemberHeldModifiers = this.scene.findModifiers(
      ...
      for (const modifier of fusedPartyMemberHeldModifiers) {
        transferModifiers.push(
          this.scene.tryTransferHeldItemModifier(modifier, this, false, modifier.getStackCount(), true, true, false),
        );
      }
      Promise.allSettled(transferModifiers).then(() => {
        this.scene.updateModifiers(true, true).then(() => {
          ...
          pokemon
            .getMoveset(true)
            .map((m: PokemonMove) =>
              this.scene.unshiftPhase(new LearnMovePhase(this.scene, newPartyMemberIndex, m.getMove().id)),
            );
          pokemon.destroy();
          this.updateFusionPalette();
          resolve();
...
export class EnemyPokemon extends Pokemon {
  ...
  setBoss(boss: boolean = true, bossSegments: integer = 0): void {
    if (boss) {
      this.bossSegments =
        bossSegments
        || this.scene.getEncounterBossSegments(this.scene.currentBattle.waveIndex, this.level, this.species, true);
  ...
  handleBossSegmentCleared(segmentIndex: integer): void {
    while (this.bossSegmentIndex > 0 && segmentIndex - 1 < this.bossSegmentIndex) {
      ...
      this.scene.unshiftPhase(
        new StatStageChangePhase(this.scene, this.getBattlerIndex(), true, [boostedStat!], stages, true, true),
      );
