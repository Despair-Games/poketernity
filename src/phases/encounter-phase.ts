...
export class EncounterPhase extends BattlePhase {
    ...
    if (mysteryEncounter) {
      ...
      if (mysteryEncounter.encounterAnimations && mysteryEncounter.encounterAnimations.length > 0) {
        loadEnemyAssets.push(
          initEncounterAnims(this.scene, mysteryEncounter.encounterAnimations).then(() =>
            loadEncounterAnimAssets(this.scene, true),
          ),
        );
      }
    ...
    battle.enemyLevels?.every((level, e) => {
      ...
      if (!this.loaded) {
        if (battle.battleType === BattleType.TRAINER) {
          ...
          if (
            this.scene.findModifier((m) => m instanceof BoostBugSpawnModifier)
          ...
          battle.enemyParty[e] = this.scene.addEnemyPokemon(
            enemySpecies,
            level,
            TrainerSlot.NONE,
      ...
      if (!this.loaded) {
        this.scene.gameData.setPokemonSeen(
          enemyPokemon,
          true,
      ...
      if (enemyPokemon.species.speciesId === Species.ETERNATUS) {
        if (
          ...
          && (battle.battleSpec === BattleSpec.FINAL_BOSS || this.scene.gameMode.isWaveFinal(battle.waveIndex))
        ) {
    ...
    Promise.all(loadEnemyAssets).then(() => {
      ...
      this.scene.ui.setMode(Mode.MESSAGE).then(() => {
        if (!this.loaded) {
          ...
          this.scene.gameData
            .saveAll(this.scene, true, battle.waveIndex % 5 === 1 || (this.scene.lastSavePlayTime ?? 0) >= 300)
            .then((success) => {
  ...
  doEncounter() {
    ...
    if (
      this.scene.isMysteryEncounterValidForWave(battleType, waveIndex)
    ...
    this.scene.tweens.add({
      targets: [
        this.scene.arenaEnemy,
        this.scene.currentBattle.trainer,
        enemyField,
        this.scene.arenaPlayer,
        this.scene.trainer,
      ].flat(),
    ...
    if (this.scene.currentBattle.battleType === BattleType.TRAINER) {
      if (this.scene.currentBattle.double) {
        return i18next.t("battle:trainerAppearedDouble", {
          trainerName: this.scene.currentBattle.trainer?.getName(TrainerSlot.NONE, true),
        });
      } else {
  ...
  doEncounterCommon(showEncounterMessage: boolean = true) {
    ...
    enemyField.forEach((enemyPokemon, e) => {
      ...
      if (
        ...
        const enemyMBH = this.scene.findModifier(
    ...
    if (![BattleType.TRAINER, BattleType.MYSTERY_ENCOUNTER].includes(this.scene.currentBattle.battleType)) {
      enemyField.map((p) =>
        this.scene.pushConditionalPhase(new PostSummonPhase(this.scene, p.getBattlerIndex()), () => {
      ...
      if (ivScannerModifier) {
        enemyField.map((p) =>
          this.scene.pushPhase(
            new ScanIvsPhase(this.scene, p.getBattlerIndex(), Math.min(ivScannerModifier.getStackCount() * 2, 6)),
          ),
        );
      }
    }
