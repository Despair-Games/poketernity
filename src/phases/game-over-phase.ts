...
export class GameOverPhase extends BattlePhase {
  ...
  start() {
    ...
    if (
      !this.isVictory &&
      this.scene.currentBattle.mysteryEncounter?.onGameOver &&
      !this.scene.currentBattle.mysteryEncounter.onGameOver(this.scene)
    ) {
      // Do not end the game
      return this.end();
    }
  ...
  handleGameOver(): void {
            ...
            if (!this.scene.ui.shouldSkipDialogue(dialogueKey)) {
              this.scene.ui.fadeIn(500).then(() => {
                ...
                this.scene.charSprite
                  .showCharacter(
                    `rival_${this.scene.gameData.gender === PlayerGender.FEMALE ? "m" : "f"}`,
    ...
    if (!Utils.isLocal || Utils.isLocalServerConnected) {
      api.savedata.session
        .newclear({ slot: this.scene.sessionSlotId, isVictory: this.isVictory, clientSessionId: clientSessionId })
  ...
  handleUnlocks(): void {
    if (this.isVictory && this.scene.gameMode.isClassic) {
      ...
      if (
        this.scene.getPlayerParty().filter((p) => p.fusionSpecies).length &&
      ...
      if (
        !this.scene.gameData.unlocks[Unlockables.EVIOLITE] &&
        this.scene.getPlayerParty().some((p) => p.getSpeciesForm(true).speciesId in pokemonEvolutions)
      ) {
  ...
  private async getRunHistoryEntry(): Promise<SessionSaveData> {
    ...
    return {
      ...
      modifiers: preWaveSessionData
        ...
        : this.scene.findModifiers(() => true).map((m) => new PersistentModifierData(m, true)),
      enemyModifiers: preWaveSessionData
        ? preWaveSessionData.enemyModifiers
        : this.scene.findModifiers(() => true, false).map((m) => new PersistentModifierData(m, false)),
