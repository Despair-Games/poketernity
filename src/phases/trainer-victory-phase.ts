...
export class TrainerVictoryPhase extends BattlePhase {
  ...
  start() {
    ...
    this.scene.unshiftPhase(
      new MoneyRewardPhase(this.scene, this.scene.currentBattle.trainer?.config.moneyMultiplier!),
    ...
    if (vouchers.hasOwnProperty(TrainerType[trainerType])) {
      if (
        ...
        && this.scene.currentBattle.trainer?.config.isBoss
      ) {
        this.scene.unshiftPhase(
    ...
    if (
      this.scene.arena.biomeType === Biome.SPACE
      ...
      () => {
        ...
        if (victoryMessages?.length) {
          if (this.scene.currentBattle.trainer?.config.hasCharSprite && !this.scene.ui.shouldSkipDialogue(message)) {
            const originalFunc = showMessageOrEnd;
            showMessageOrEnd = () =>
              this.scene.charSprite.hide().then(() => this.scene.hideFieldOverlay(250).then(() => originalFunc()));
            this.scene
