...
export class SwitchSummonPhase extends SummonPhase {
  ...
  preSummon(): void {
    ...
    if (
      !this.doReturn
      || (this.slotIndex !== -1
        && !(this.player ? this.scene.getPlayerParty() : this.scene.getEnemyParty())[this.slotIndex])
    ) {
      if (this.player) {
    ...
    (this.player ? this.scene.getEnemyField() : this.scene.getPlayerField()).forEach((enemyPokemon) =>
    ...
    this.scene.ui.showText(
      this.player
        ...
        : i18next.t("battle:trainerComeBack", {
            trainerName: this.scene.currentBattle.trainer?.getName(
  ...
  switchAndSummon() {
    ...
    if (this.switchType === SwitchType.BATON_PASS && switchedInPokemon) {
      ...
      if (
        ...
        const batonPassModifier = this.scene.findModifier(
          (m) =>
            m instanceof SwitchEffectTransferModifier
        ...
        if (
          ...
          this.scene.tryTransferHeldItemModifier(
            batonPassModifier,
            switchedInPokemon,
            false,
            undefined,
