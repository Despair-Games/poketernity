...
import { ExpGainSpeed, expGainSpeedMap } from "#enums/exp-gain-speed";
    ...
    if (partyExpNotificationMode === ExpNotification.SKIP) {
      ...
      if (newLevel > lastLevel) {
        ...
        partyExpBar
          ...
          .then(() => {
            setTimeout(() => this.end(), 800 * expGainSpeedMap[expGainSpeed]);
          });
      } else {
        this.end();
      }
      ...
      partyExpBar.showPokemonExp(pokemon, exp.value, false, newLevel).then(() => {
        setTimeout(() => this.end(), 500 * expGainSpeedMap[expGainSpeed]);
      });
    } else {
      this.end();
    }
