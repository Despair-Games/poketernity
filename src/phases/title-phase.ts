...
export class TitlePhase extends Phase {
    ...
    options.push(
      {
        ...
        handler: () => {
          ...
          if (gameData.isUnlocked(Unlockables.ENDLESS_MODE)) {
            ...
            this.scene.ui.showText(i18next.t("menu:selectGameMode"), null, () =>
              this.scene.ui.setOverlayMode(Mode.OPTION_SELECT, { options: options }),
            );
          } else {
      ...
      },
      {
        ...
        handler: () => {
          this.scene.ui.setOverlayMode(Mode.SAVE_SLOT, SaveSlotUiMode.LOAD, (slotId: integer) => {
            if (slotId === -1) {
  ...
  loadSaveSlot(slotId: integer): void {
    ...
    this.scene.gameData
      .loadSession(this.scene, slotId, slotId === -1 ? this.lastSessionData : undefined)
